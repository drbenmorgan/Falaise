...
const manager &detector_construction::get_manager() const {
  DT_THROW_IF(_g4_manager_ == nullptr, std::logic_error,
              "Operation prohibited !");
  return *_g4_manager_;
}

manager &detector_construction::grab_manager() {
  DT_THROW_IF(_g4_manager_ == nullptr, std::logic_error,
              "Operation prohibited !");
  return *_g4_manager_;
}

const geomtools::manager &detector_construction::get_geometry_manager() const {
...
void detector_construction::_set_default() {
  ...
  _SD_params_.set_description(
      "Sensitive detectors' configuration "
...
bool detector_construction::is_initialized() const { return _initialized_; }

bool detector_construction::has_geometry_manager() const {
  return _geom_manager_ != 0;
}

void detector_construction::set_geometry_manager(
    const geomtools::manager &gm_) {
...
void detector_construction::reset() {
  ...
  for (std::map<std::string, G4VisAttributes *>::iterator i =
           _vis_attributes_.begin();
       i != _vis_attributes_.end(); ++i) {
    if (i->second != 0) {
      delete i->second;
      i->second = 0;
    }
...
void detector_construction::initialize(const datatools::properties &config_) {
  ...
  if (config_.has_key("gdml.schema_location")) {
    std::string gdml_schema_location =
        config_.fetch_string("gdml.schema_location");
  ...
  config_.export_and_rename_starting_with(_SHPF_config_,
  ...
  if (config_.has_key("sensitive.detectors")) {
    ...
    for (unsigned int i = 0; i < list_of_sensitive_detectors.size(); ++i) {
      ...
      config_.export_and_rename_starting_with(
          _SD_params_.grab(SD_category).grab_properties(), key_oss,
          new_key_oss);
    }  // for

    DT_LOG_NOTICE(_logprio(), "Info for sensitive detectors : ");
    for (std::vector<std::string>::const_iterator i =
             list_of_sensitive_detectors.begin();
  ...
  if (config_.has_key("using_electromagnetic_field")) {
    ...
    DT_LOG_DEBUG(_logprio(),
                 "Using electromagnetic field(s) : " << _using_em_field_);
  } else if (config_.has_key("using_magnetic_field")) {
    // Obsolete:
    DT_LOG_WARNING(datatools::logger::PRIO_ALWAYS,
                   "Configuration property 'using_magnetic_field' is obsolete, "
                   "use 'using_electromagnetic_field'!");
  ...
  if (_using_em_field_) {
    ...
    if (config_.has_key("electromagnetic_field.plugin_name")) {
      std::string fpn =
          config_.fetch_string("electromagnetic_field.plugin_name");
      set_emfield_geom_plugin_name(fpn);
    } else if (config_.has_key("magnetic_field.plugin_name")) {
      // Obsolete:
      DT_LOG_WARNING(datatools::logger::PRIO_ALWAYS,
                     "Configuration property 'magnetic_field.plugin_name' is "
                     "obsolete, use 'electromagnetic_field.plugin_name'!");
      _using_em_field_ = config_.fetch_boolean("using_magnetic_field");
      std::string fpn = config_.fetch_string("magnetic_field.plugin_name");
      set_emfield_geom_plugin_name(fpn);
    }

    DT_LOG_DEBUG(_logprio(), "Electromagnetic fields geometry plugin = '"
                                 << _emfield_geom_plugin_name_ << "'");
    ...
    if (config_.has_key("electromagnetic_field.miss_distance.unit")) {
      ...
      _miss_distance_unit_ =
          datatools::units::get_length_unit_from(md_unit_str);
    } else if (config_.has_key("magnetic_field.miss_distance.unit")) {
      // Obsolete:
      DT_LOG_WARNING(datatools::logger::PRIO_ALWAYS,
                     "Configuration property "
                     "'magnetic_field.miss_distance.unit' is obsolete, use "
                     "'electromagnetic_field.miss_distance.unit'!");
      std::string md_unit_str =
          config_.fetch_string("magnetic_field.miss_distance.unit");
      _miss_distance_unit_ =
          datatools::units::get_length_unit_from(md_unit_str);
    }

    if (config_.has_key("electromagnetic_field.miss_distance")) {
      _general_miss_distance_ =
          config_.fetch_real("electromagnetic_field.miss_distance");
      ...
      // Obsolete:
      DT_LOG_WARNING(datatools::logger::PRIO_ALWAYS,
                     "Configuration property 'magnetic_field.miss_distance' is "
                     "obsolete, use 'electromagnetic_field.miss_distance'!");
      _general_miss_distance_ =
          config_.fetch_real("magnetic_field.miss_distance");
      if (!config_.has_explicit_unit("magnetic_field.miss_distance")) {
        _general_miss_distance_ *= _miss_distance_unit_;
      }
    }
    DT_LOG_DEBUG(_logprio(), "General miss distance = "
                                 << _general_miss_distance_ / CLHEP::mm
                                 << " mm");
    ...
    config_.export_and_rename_starting_with(_em_field_aux_,
...
detector_construction::get_em_field_manager() const {
  DT_THROW_IF(_em_field_manager_ == 0, std::logic_error,
...
void detector_construction::_construct_electromagnetic_field() {
  ...
  DT_LOG_DEBUG(logging, "Has EM field geom map : "
                            << (has_em_field_manager() &&
  ...
  if (has_em_field_manager() && _em_field_manager_->has_geom_map()) {
    ...
    for (gma_type::const_iterator i = mfamap.begin(); i != mfamap.end(); i++) {
      ...
      G4LogicalVolume *g4_module_log =
          g4_LV_store->GetVolume(g4_log_name, false);
      if (g4_module_log == 0) {
        std::ostringstream message;
      ...
      em_field_g4_stuff *emf_working =
          _em_field_g4_data_.find(association_name)->second;
      if (!gefa.field->is_magnetic_field() &&
          !gefa.field->is_electric_field()) {
        DT_THROW(std::logic_error,
                 "Field '" << association_name
                           << "' is not an electromagnetic field !");
      }
      if (gefa.field->is_magnetic_field() && !gefa.field->is_electric_field()) {
        ...
        DT_LOG_NOTICE(logging, "Initializing pure magnetic field with label '"
                                   << association_name << "'...");
      } else {
      ...
      DT_LOG_NOTICE(logging, "G4 logical volume '"
                                 ...
                                 << gefa.get_field_name() << "' !");
      DT_LOG_NOTICE(logging,
                    "Miss distance is " << miss_distance / CLHEP::mm << " mm");
    }
...
void detector_construction::_construct_sensitive_detectors() {
  ...
  for (geomtools::logical_volume::dict_type::const_iterator ilogical =
    ...
    if (geomtools::sensitive::is_sensitive(log.get_parameters())) {
      DT_LOG_DEBUG(_logprio(),
                   "Logical volume '" << log.get_name() << "' is sensitive !");
  ...
  for (datatools::multi_properties::entries_col_type::const_iterator i =
    ...
    if (sd_config.has_key("sensitive.volumes")) {
      ...
      for (int j = 0; j < (int)lv_names.size(); j++) {
        ...
        // Check if the logical volume association matches the proper SD
        // configuration
        DT_THROW_IF(
            lv_sd_associations.find(lv_name) != lv_sd_associations.end() &&
                lv_sd_associations[lv_name] != sensitive_category_name,
            std::logic_error,
            "Logical volume '" << lv_name
                               << "' is already associated to category '"
  ...
  for (std::map<std::string, std::string>::const_iterator i =
    ...
    //           << std::endl;
    // We throw if some logical volumes do not exist and thus cannot be
    // associated to a given requested sensitive category (error in some
    // configuration file):
    DT_THROW_IF(_geom_manager_->get_factory().get_logicals().find(lv_name) ==
                    _geom_manager_->get_factory().get_logicals().end(),
                std::logic_error,
                "Logical volume '" << lv_name
                                   << "' does not exists (supposed to be "
                                      "associated to sensitive category '"
                                   << sd_name << "'!");
    // DT_THROW_IF(! _SD_params_.has_key(sd_name),
    //             std::logic_error,
    //             "Sensitive category '" << sd_name << "' does not exists
    //             (supposed to be associated to logical volume '"
    //             << lv_name << "'!");
    if (!_SD_params_.has_key(sd_name)) {
      // Is some logical volume cannot find its associated sensitive category,
      // we only warn:
      DT_LOG_WARNING(_logprio(), "Sensitive category '"
                                     << sd_name
                                     << "' does not exists but is supposed to "
                                        "be associated to logical volume '"
                                     << lv_name
                                     << "'! Ignoring this association !");
    }
  }

  // Loop on logical volumes, processing only ones associated to some sensitive
  // category:
  for (geomtools::logical_volume::dict_type::const_iterator ilogical =
           _geom_manager_->get_factory().get_logicals().begin();
       ilogical != _geom_manager_->get_factory().get_logicals().end();
    ...
    if (lv_sd_associations.find(log.get_name()) == lv_sd_associations.end()) {
      DT_LOG_DEBUG(_logprio(), "Logical volume '" << log.get_name()
                                                  << "' is not sensitive !");
      continue;
    }
    ...
    // if (! geomtools::sensitive::is_sensitive(log.get_parameters())) {
    //   DT_LOG_DEBUG(_logprio(),"Logical volume '" << log.get_name() << "' is
    //   not sensitive !"); continue;
    // }
    ...
    DT_LOG_NOTICE(_logprio(), "Logical volume '"
                                  << log.get_name()
    ...
    _sensitive_detectors_[sensitive_category]->attach_logical_volume(
        log.get_name());
  ...
  DT_LOG_NOTICE(_logprio(),
                "SHPF: Preparing the step hit processor factory (SHPF)...");
  // Logging priority:
  ...
  if (_g4_manager_ != nullptr) {
    _SHPF_.set_output_profiles(
        _g4_manager_->get_activated_output_profile_ids());
  } else {
  ...
  DT_THROW_IF(
      config_files.size() == 0, std::logic_error,
  ...
  for (size_t i = 0; i < config_files.size(); i++) {
    ...
    DT_LOG_NOTICE(_logprio(),
                  "SHPF: The SHPF configuration file has been parsed.");
    DT_LOG_NOTICE(_logprio(),
                  "SHPF: Loading the hit processors from the SHPF...");
    _SHPF_.load(mconfig);
  }
  _SHPF_.initialize();
  DT_LOG_NOTICE(
      _logprio(),
      "SHPF: The step hit processors have been loaded from the SHPF.");
  ...
  for (SHPF_type::processor_dict_type::const_iterator iSHP =
       ...
       iSHP != _SHPF_.get_processors().end(); ++iSHP) {
    ...
    if (_sensitive_detectors_.find(from_processor_sensitive_category) !=
      ...
      sensitive_detector *SD =
          new sensitive_detector(from_processor_sensitive_category);
      if (_g4_manager_ != nullptr) {
        SD->set_manager(*_g4_manager_);
      } else {
        DT_LOG_FATAL(_logprio(),
                     "Missing G4 manager for sensitive detectors!!!");
      }
      ...
      if (all_logicals) {
        for (geomtools::logical_volume::dict_type::const_iterator ilogical =
          ...
          if (std::find(logicals.begin(), logicals.end(), logical_name) ==
              logicals.end()) {
      ...
      // Fetch the list of logical volumes associated to this 'sensitive
      // category' :
      if (!all_logicals) {
        if (processor->get_auxiliaries().has_key("geometry.volumes")) {
          DT_LOG_NOTICE(_logprio(),
                        "SHPF: Fetch the list of logical volumes associated to "
      ...
      // Also fetch the list of logical volumes associated to this 'sensitive
      // category' and *with* a given list of materials :
      if (processor->get_auxiliaries().has_key(
              "geometry.volumes.with_materials")) {
        ...
        DT_LOG_NOTICE(
            ...
            "SHPF: Fetch the list of materials for sensitive category '"
                << from_processor_sensitive_category << "'");
        processor->get_auxiliaries().fetch("geometry.volumes.with_materials",
                                           materials);
        ...
        message << ") to be associated to the new sensitive detector with "
                   "category '"
                << from_processor_sensitive_category << "'";
      ...
      if (processor->get_auxiliaries().has_key("geometry.volumes.excluded")) {
        ...
        processor->get_auxiliaries().fetch("geometry.volumes.excluded",
                                           excluded_logicals);
        std::ostringstream message;
        ...
        message << ") from the list of volumes to be associated to the new "
                   "sensitive detector from SHPF with category '"
                << from_processor_sensitive_category << "'";
        DT_LOG_NOTICE(_logprio(), message.str());

        for (size_t ixm = 0; ixm < excluded_logicals.size(); ixm++) {
          logicals.erase(std::remove(logicals.begin(), logicals.end(),
                                     excluded_logicals[ixm]),
                         logicals.end());
        }
      }
      ...
      for (std::vector<std::string>::const_iterator ilogical = logicals.begin();
        ...
        if (geomtools::sensitive::is_sensitive(log.get_parameters())) {
          DT_LOG_WARNING(
              ...
              "SHPF: Logical volume '"
                  << logical_name
                  << "' is already associated to sensitive category '"
                  << geomtools::sensitive::get_sensitive_category(
                         log.get_parameters())
                  << "' !");
          continue;
        }
        ...
        // Local scope:
        {
          // Makes the logical sensitive (using a trick because of the
          // const-ness of the logical volume instance after building the model
          // factory from the geometry manager) :
          geomtools::logical_volume *mutable_log =
              const_cast<geomtools::logical_volume *>(&log);
          geomtools::sensitive::set_sensitive_category(
              mutable_log->grab_parameters(),
              from_processor_sensitive_category);
        }
        ...
        message << "SHPF: Make the G4 logical volume '" << log.get_name()
                ...
                << from_processor_sensitive_category << "'";
        DT_LOG_NOTICE(_logprio(), message.str());
        _sensitive_detectors_[from_processor_sensitive_category]
            ->attach_logical_volume(log.get_name());

        g4_log->SetSensitiveDetector(
            _sensitive_detectors_[from_processor_sensitive_category]);
      }
    }
  }  // for (hit processors)
  ...
  for (SHPF_type::processor_dict_type::const_iterator iSHP =
    ...
    const std::string &from_processor_sensitive_category =
        processor->get_sensitive_category();
  ...
  if (is_verbose()) {
    for (detector_construction::sensitive_detector_dict_type::iterator iSD =
         ...
         iSD != _sensitive_detectors_.end(); ++iSD) {
      const std::string &sensitive_category = iSD->first;
      sensitive_detector *SD = iSD->second;
      DT_LOG_NOTICE(_logprio(),
                    "Sensitive detector '" << sensitive_category << "' : ");
...
void detector_construction::set_materials_geom_plugin_name(
    const std::string &mpn_) {
  DT_THROW_IF(is_initialized(), std::logic_error, "Operation prohibited !");
  _materials_geom_plugin_name_ = mpn_;
  return;
}

void detector_construction::set_emfield_geom_plugin_name(
    const std::string &fpn_) {
  DT_THROW_IF(is_initialized(), std::logic_error, "Operation prohibited !");
...
void detector_construction::write_tmp_gdml_file() {
  ...
  DT_THROW_IF(err == -1, std::runtime_error,
              "Cannot create GDML temporary file !");
  ...
  if (!materials_plugin_name.empty()) {
    DT_LOG_NOTICE(_logprio(), "Requested materials plugin name is '"
    ...
    DT_LOG_NOTICE(
        _logprio(),
        "Trying to find a materials plugin associated to the manager...");
    typedef geomtools::manager::plugins_dict_type dict_type;
    const dict_type &plugins = geo_mgr.get_plugins();
    for (dict_type::const_iterator ip = plugins.begin(); ip != plugins.end();
         ip++) {
  ...
  if (geo_mgr.has_plugin(materials_plugin_name) &&
    ...
    DT_LOG_NOTICE(_logprio(), "Found materials plugin named '"
                                  << materials_plugin_name << "'");
  ...
  DT_THROW_IF(mat_mgr_ref == 0, std::logic_error,
              "Cannot find a material plugin in the geometry manager !");
  DT_LOG_NOTICE(_logprio(),
                "Export GDML materials from the materials plugin: ");
  ...
  DT_LOG_TRACE(
      ...
      "Materials writer: \n"
          << materials_writer
                 .get_stream(geomtools::gdml_writer::materials_section())
                 .str());
...
void detector_construction::_set_user_limits() {
  ...
  for (std::map<std::string, std::string>::const_iterator i = targets.begin();
       i != targets.end(); i++) {
    DT_LOG_NOTICE(_logprio(),
                  "Fetching user limits for G4 logical volume/region '"
                      << i->first << "'...");
    ...
    if (i->second == "logical_volume") {
      ...
      if (found == _geom_manager_->get_factory().get_logicals().end()) {
        DT_THROW_IF(true, std::logic_error,
                    "Cannot find logical volume '" << target_name << "' for user
      limits !");
      }
      */
      DT_LOG_NOTICE(_logprio(), "Searching G4 logical volume '"
                                    << target_name.c_str() << "'...");
      g4_logical = G4LogicalVolumeStore::GetInstance()->GetVolume(
      ...
      DT_LOG_NOTICE(_logprio(),
                    "Searching G4 region '" << target_name.c_str() << "'...");
    ...
    if (ulimits_used) {
      // 2014-12-12, FM:
      // max_step implies G4StepLimiter pseudo process to be associated to the
      // particle. Other params imply G4UserSpecialCuts pseudo process to be
      // associated to the particle (not implemented yet).
      G4UserLimits *g4_user_limits =
          new G4UserLimits(max_step, max_track_length, max_track_time,
...
void detector_construction::_construct_regions() {
  ...
  // {
  //   for (std::vector<G4LogicalVolume*>::const_iterator i =
  //   g4_LV_store->begin();
  //        i != g4_LV_store->end();
  //        i++) {
  ...
  for (region_infos_dict_type::const_iterator i =
       ...
       i != _region_definitions_.get_region_infos().end(); ++i) {
    ...
    for (std::set<std::string>::const_iterator j =
             the_region_info.get_logical_ids().begin();
         j != the_region_info.get_logical_ids().end(); j++) {
      std::string logical_volume_name = *j;
      G4String g4_logical_volume_name = logical_volume_name.c_str();
      G4LogicalVolume *a_logical =
          g4_LV_store->GetVolume(g4_logical_volume_name, false);
  ...
  for (geomtools::logical_volume::dict_type::const_iterator ilogical =
    ...
    if (geomtools::visibility::is_hidden(log.get_parameters())) {
      ...
      if (geomtools::visibility::has_opacity(log.get_parameters())) {
        display_opacity =
            geomtools::visibility::get_opacity(log.get_parameters());
    ...
    if (g4_log != 0) {
      ...
      if (!display_color.empty()) {
        const geomtools::color::constants &gcc =
            geomtools::color::constants::instance();
      ...
      if (visible) {
        va->SetColor(G4Colour(color.GetRed(), color.GetGreen(), color.GetBlue(),
                              display_opacity));
      }
      ...
      DT_LOG_DEBUG(_logprio(), "Cannot find logical volume '"
                                   << log.get_name()
...
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(mctools::g4::detector_construction, ocd_) {
  ...
  ocd_.set_class_description(
      "The Geant4 simulation mandatory detector construction");
  ...
  ocd_.set_class_documentation(
      "The Geant4 simulation engine/manager needs to be given a *detector "
      "construction* \n"
      "object. This complex object is responsible of some fundamental "
      "operations\n"
      ...
      " \n"
      " * initialization of the *step hit processor factory* for "
      "post-processing\n"
...

  {
    ...
    cpd.set_name_pattern("gdml.tmp_dir")
        .set_terse_description(
            "The directory where the GDML file is generated by the "
            "``geomtools::manager``")
...

  {
    ...
    cpd.set_name_pattern("gdml.validation")
        .set_terse_description(
            "Flag to proceed to the validation of the GDML file")
...

  {
    ...
    cpd.set_name_pattern("gdml.schema_location")
        .set_terse_description(
            "The label indicating where to find the GDML schema resources")
        .set_traits(datatools::TYPE_STRING)
        .set_long_description(
...

  {
    ...
    cpd.set_name_pattern("materials.plugin_name")
        .set_terse_description(
            "The name of the materials plugin embedded in the geometry manager "
            "for materials export in GDML")
        .set_traits(datatools::TYPE_STRING)
        .set_long_description(
            "Default value: empty (plugin is autodetected from the geometry "
            "manager)\n")
        .add_example(
...

  {
    ...
    cpd.set_name_pattern("hit_processor_factory.config")
        ...
        .set_long_description(
            "Default value: empty                                   \n"
            "                                                       \n"
            "This property is not parsed if the "
            "\"hit_processor_factory.configs\" \n"
            "is provided.                                           \n"
            "                                                       \n")
        .add_example(
...

  {
    ...
    cpd.set_name_pattern("sensitive.detectors")
        .set_terse_description(
            "The list of sensitive detectors attached to the geometry")
        .set_traits(datatools::TYPE_STRING,
                    configuration_property_description::ARRAY)
        .set_long_description(
            "This is the list of so-called *official* sensitive detectors in "
            "the   \n"
            "geometry setup. Each sensitive detectors is identified by its "
            "unique  \n"
            "name.                                                             "
            "    \n")
        .add_example(
            ...
            "    \n"
            "and ``KEY1``, ``TYPE1`` and  ``VALUE1`` are the property's "
            "identifier, type \n"
            "and value respectively. Please refer to the OCD manual of the     "
            "    \n"
            "``mctools::g4::sensitive_detector`` class for further "
            "documentation   \n"
            "about the available configuration parameters for *sensitive "
            "detector* objects.\n");
  }

  {
    ...
    cpd.set_name_pattern("sensitive.${sensitive.detectors}.record_track_id")
        .set_terse_description(
            "Record the *track ID* attached to a given sensitive detector")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_default_value_boolean(false)
        .set_long_description(
            "See OCD manual of the ``mctools::g4::sensitive_detector`` "
            "class.\n")
        .add_example(
  ...
  {
    // Description of the 'sensitive.XXX.record_primary_particle' configuration
    // property :
    datatools::configuration_property_description &cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern(
           "sensitive.${sensitive.detectors}.record_primary_particle")
        .set_terse_description(
            "Record *primary particle* flag attached to a given sensitive "
            "detector")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_default_value_boolean(false)
        .set_long_description(
  ...
  {
    // Description of the 'sensitive.XXX.record_alpha_quenching' configuration
    // property :
    datatools::configuration_property_description &cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern(
           "sensitive.${sensitive.detectors}.record_alpha_quenching")
        .set_terse_description(
            "Record the *alpha quenching* flag attached to a given sensitive "
            "detector")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_default_value_boolean(false)
        .set_long_description(
  ...
  {
    // Description of the 'sensitive.XXX.record_major_track' configuration
    // property :
    datatools::configuration_property_description &cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern("sensitive.${sensitive.detectors}.record_major_track")
        .set_terse_description(
            "Record the *major track* flag attached to a given sensitive "
            "detector")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_default_value_boolean(false)
        .set_long_description(
  ...
  {
    // Description of the 'sensitive.XXX.major_track_minimum_energy'
    // configuration property :
    datatools::configuration_property_description &cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern(
           "sensitive.${sensitive.detectors}.major_track_minimum_energy")
        .set_terse_description(
            "Energy threshold to consider a track as a *major* one")
        ...
        .add_example(
            ...
            "  sensitive.calo_SD.record_major_track         : boolean = 1      "
            "        \n"
            "  sensitive.calo_SD.major_track_minimum_energy : real as energy = "
            "50 keV  \n"
            "                                                                  "
            "        \n");
  }

  {
    ...
    cpd.set_name_pattern("sensitive.${sensitive.detectors}.record_category")
        .set_terse_description(
            "Record the *sensitive category* attached to a given sensitive "
            "detector")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_default_value_boolean(false)
        .set_long_description(
...

  {
    ...
    cpd.set_name_pattern("sensitive.${sensitive.detectors}.record_momentum")
        .set_terse_description(
            "Record the start and stop *momenta* of the step for a given "
            "sensitive detector")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_default_value_boolean(false)
        .set_long_description(
  ...
  {
    // Description of the 'sensitive.XXX.record_kinetic_energy' configuration
    // property :
    datatools::configuration_property_description &cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern(
           "sensitive.${sensitive.detectors}.record_kinetic_energy")
        .set_terse_description(
            "Record the start and stop *kinetic energy* of the step for a "
            "given sensitive detector")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_default_value_boolean(false)
        .set_long_description(
            "See OCD manual of the ``mctools::g4::sensitive_detector`` "
  ...
  {
    // Description of the 'sensitive.XXX.hits_buffer_capacity' configuration
    // property :
    datatools::configuration_property_description &cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern(
           "sensitive.${sensitive.detectors}.hits_buffer_capacity")
        .set_terse_description(
            "Pre-allocate the initial capacity of a buffer of step hits "
  ...
  {
    // Description of the 'sensitive.XXX.drop_zero_energy_deposit_steps'
    // configuration property :
    datatools::configuration_property_description &cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern(
           "sensitive.${sensitive.detectors}.drop_zero_energy_deposit_steps")
        .set_terse_description(
            "Drop zero energy deposit step hits for a given sensitive detector")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_default_value_boolean(false)
        .set_long_description(
            "See OCD manual of the ``mctools::g4::sensitive_detector`` "
        ...
        .add_example(
            ...
            "  sensitive.detectors : string[2] = \"calo_SD\" \"tracker_SD\"    "
            "    \n"
            "  sensitive.calo_SD.drop_zero_energy_deposit_steps    : boolean = "
            "1   \n"
            "  sensitive.tracker_SD.drop_zero_energy_deposit_steps : boolean = "
            "0   \n"
            "                                                                  "
            "    \n");
  }
  ...
  {
    // Description of the 'sensitive.XXX.record_g4_volume_infos' configuration
    // property :
    datatools::configuration_property_description &cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern(
           "sensitive.${sensitive.detectors}.record_g4_volume_infos")
        .set_terse_description(
            "Record the Geant4 geometry volume informations for a given "
            "sensitive detector")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_default_value_boolean(false)
        .set_long_description(
...

  {
    ...
    cpd.set_name_pattern("regions.${regions}.volumes")
        .set_terse_description(
            "The list of volumes attached to a region referenced through its "
            "name")
        .set_traits(datatools::TYPE_STRING,
                    configuration_property_description::ARRAY)
        .set_mandatory(false)
        .add_example(
            ...
            "  regions : string[2] = \"calo\" \"tracker\"                      "
            "    \n"
            "  regions.calo.volumes    : string[2] = \"calo0.log\" "
            "\"calo1.log\"   \n"
  ...
  {
    // Description of the 'regions' volumes.with_material' configuration
    // property :
    datatools::configuration_property_description &cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern("regions.${regions}.volumes.with_materials")
        .set_terse_description(
            "The list of volumes with some given materials attached to a "
            "region referenced through its name")
        .set_traits(datatools::TYPE_STRING,
                    configuration_property_description::ARRAY)
        .set_mandatory(false)
        .add_example(
            ...
            "  regions : string[2] = \"calo\" \"tracker\"                      "
            "       \n"
            "  regions.calo.volumes.with_materials    : string[2] = \"PVT\" "
            "\"NE110\" \n"
            "  regions.tracker.volumes.with_materials : string = "
            "\"tracking_gas\"     \n"
            "                                                                  "
            "       \n");
  }
...

  {
    ...
    cpd.set_name_pattern("limits.list_of_volumes")
        ...
        .add_example(
            ...
            "                                                                  "
            "             \n"
            "  limits.list_of_volumes : string[2] = \"calo_block.log\" "
            "\"tracker_cell.log\" \n"
            "                                                                  "
            "             \n");
  }

  {
    ...
    cpd.set_name_pattern("limits.max_step.${limits.list_of_volumes}")
        .set_terse_description(
            "The maximum tracking step length associated to a given volume")
        .set_traits(datatools::TYPE_REAL)
        .set_explicit_unit(true)
...

  {
    ...
    cpd.set_name_pattern("magnetic_field.plugin_name")
        .set_terse_description(
            "The name of the geometry magnetic field plugin embedded in the "
            "geometry manager")
        .set_traits(datatools::TYPE_STRING)
        .set_mandatory(true)
  ...
  {
    // Description of the 'magnetic_field.miss_distance.unit' configuration
    // property :
    datatools::configuration_property_description &cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern("magnetic_field.miss_distance.unit")
        .set_terse_description(
            "The default implicit magnetic field miss distance unit")
        .set_traits(datatools::TYPE_STRING)
        .set_mandatory(false)
        .set_triggered_by_flag("using_magnetic_field")
...

  {
    ...
    cpd.set_name_pattern("magnetic_field.logging.priority")
        .set_terse_description(
            "Logging priority threshold of the magnetic field manager")
        .set_traits(datatools::TYPE_STRING)
        .set_mandatory(false)
...

  {
    ...
    cpd.set_name_pattern("magnetic_field.associations")
        .set_terse_description(
            "List of geom./mag. field associations with special criteria")
        .set_traits(datatools::TYPE_STRING,
                    configuration_property_description::ARRAY)
        .set_mandatory(false)
        .add_example(
            ...
            "                                                                  "
            "  \n"
            "Note: the geometry volume/magnetic field association must exist "
            "in  \n"
  ...
  {
    // Description of the 'magnetic_field.check_pos_time.XXX' configuration
    // property :
    datatools::configuration_property_description &cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern(
           "magnetic_field.miss_distance.${magnetic_field.associations}")
        .set_terse_description(
            "Set a specific miss distance for a given geom/mag. field "
            "association")
        .set_traits(datatools::TYPE_REAL)
        .set_mandatory(false)
        .set_explicit_unit(true)
        .set_unit_label("length")
        .set_long_description(
            "Apply a special miss distance for a given geom. volume/mag. field "
            "association. \n")
        .add_example(
            "Example::                                                         "
            "  \n"
            "                                                                  "
            "  \n"
            "  magnetic_field.associations       : string[2] = \"coil\" "
            "\"lab\"  \n"
...

  {
    ...
    datatools::configuration_property_description &cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern(
           "magnetic_field.check_pos_time.${magnetic_field.associations}")
        .set_terse_description(
            "Flag to force the checking of position/time while computing mag. "
        ...
        .set_long_description(
            ...
            "   \n"
            "magnetic field referenced by a given geom. volume/mag. field "
            "association. \n")
        .add_example(
            "Example::                                                         "
            "  \n"
  ...
  ocd_.set_configuration_hints(
      ...
      "     \n"
      "  #@config Main configuration file for the detector construction user "
      "object \n"
