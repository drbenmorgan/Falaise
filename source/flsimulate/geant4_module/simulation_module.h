...
class simulation_module : public dpp::base_module {
 public:
  /// Constructor
  simulation_module(datatools::logger::priority logging_priority =
  ...
  /// Set the geometry service label (only used if no geometry manager is
  /// provided)
  void set_geo_label(const std::string&);

  /// Get the geometry service label (only used if no geometry manager is
  /// provided)
 ...
 private:
  std::string geometryServiceName_;  //!< The label of the Geometry service to
                                     //!be accessed
  std::string simdataBankName_;      //!< The label of the 'simulated_data' bank in
                                     //!the event record

  const geomtools::manager*
      geometryManagerRef_;  //!< Non-mutable reference to the geometry manager

  manager_parameters geant4Parameters_;  //!< The configuration parameters for
                                         //!the GEANT4 simulation manager
  manager* geant4Simulation_;  //!< The embedded GEANT4 simulation manager
  simulation_ctrl* geant4SimulationController_;  //!< The embedded control
                                                 //!object for thread
                                                 //!synchronization
