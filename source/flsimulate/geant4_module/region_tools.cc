...
bool region_info::is_initialized() const { return _initialized_; }
...
void region_info::initialize(const datatools::properties& config_,
  ...
  if (geom_manager_ != nullptr) {
    ...
    bool use_materials = false;  // volume use a specific material from an
                                 // explicit list of materials
    bool use_density_range =
        false;  // volume use a material with density in a specific range
    bool use_state = false;  // volume use a material with a specific state
                             // (solid/liquid/gas)
    ...
    // Search for logical volumes with specific materials with density in a
    // given range:
    ...
    if (!use_materials) {
      if (config_.has_key("material.max_density")) {
        requested_material_max_density =
            config_.fetch_real("material.max_density");
      ...
      if (config_.has_key("material.requested_material_min_density")) {
        requested_material_min_density =
            config_.fetch_real("material.min_density");
      ...
      if (use_density_range) {
        ...
        DT_THROW_IF(
            requested_material_min_density >= requested_material_max_density,
            std::range_error,
    ...
    if (!use_materials && !use_density_range) {
      if (config_.has_key("material.state")) {
        ...
        requested_material_state =
            materials::material::label_to_state(state_label);
        DT_THROW_IF(
            requested_material_state != materials::material::STATE_UNKNOWN,
            std::logic_error,
    ...
    {
      // Attempt to access to some embedded material manager from the geometry
      // manager:
      std::string materials_plugin_name;
      DT_LOG_DEBUG(get_logging_priority(),
                   "Trying to find a materials plugin associated to the "
                   "geometry manager...");
      ...
      for (dict_type::const_iterator ip = plugins.begin(); ip != plugins.end();
        ...
        if (geom_manager_->is_plugin_a<geomtools::materials_plugin>(
                plugin_name)) {
      ...
      if (geom_manager_->has_plugin(materials_plugin_name) &&
          geom_manager_->is_plugin_a<geomtools::materials_plugin>(
              materials_plugin_name)) {
        DT_LOG_NOTICE(get_logging_priority(), "Found materials plugin named '"
                                                  << materials_plugin_name
                                                  << "'");
        const geomtools::materials_plugin& mgp =
            geom_manager_->get_plugin<geomtools::materials_plugin>(
                materials_plugin_name);
    ...
    for (geomtools::logical_volume::dict_type::const_iterator ilogical =
      ...
      if (!mr.empty()) {
        if (use_materials) {
          ...
          const materials::material& mat =
              mat_mgr_ref->get_material(effective_mat_ref);

          if (use_density_range) {
            // Check if the density of the material of the logical volume is in
            // the range:
            ...
            // Check if the state of the material of the logical volume matches
            // the requested one:
  ...
  if (config_.has_key("volumes.excluded")) {
    // std::cerr << "DEVEL: Region '" << get_name() << "' : checking excluded
    // volumes..." << std::endl;
    ...
    for (size_t i = 0; i < excluded_volumes.size(); i++) {
      ...
      // std::cerr << "DEVEL: Region '" << get_name() << "' : processing
      // excluded volume '" << log_name << "' ..." << std::endl;
      if (has_logical_volume(log_name)) {
        // std::cerr << "DEVEL: Region '" << get_name() << "' : removing logical
        // '" << log_name << "'..." << std::endl;
        remove_logical_volume(log_name);
      }
    }
  } else {
    // std::cerr << "DEVEL: Region '" << get_name() << "' : no excluded
    // volumes." << std::endl;
  }
  // config_.tree_dump(std::cerr, "Region info config: ", "DEVEL: ");

  DT_THROW_IF(_logical_ids_.size() == 0, std::logic_error,
...
void region_info::add_logical_volume(const std::string& logical_id_,
                                     bool allow_duplicate_) {
  DT_THROW_IF(
      !allow_duplicate_ && _logical_ids_.count(logical_id_), std::logic_error,
...

{
  DT_THROW_IF(!has_logical_volume(logical_id_), std::logic_error,
              "Region '" << get_name() << "' has no logical volume named '"
                         << logical_id_ << "'!");
...
std::set<std::string>& region_info::grab_logical_ids() { return _logical_ids_; }
...
const geomtools::manager& regions_setup::get_geometry_manager() const {
  DT_THROW_IF(_geom_manager_ == 0, std::logic_error,
              "No geoemtry manager is referenced!");
...
void regions_setup::initialize(const datatools::properties& config_) {
  ...
  for (unsigned int i = 0; i < regions.size(); ++i) {
    ...
    config_.export_and_rename_starting_with(region_config,
                                            region_key_prefix.str(), "");
...
datatools::command::returned_info regions_setup::_check() {
  ...
  for (region_infos_dict_type::const_iterator i = _region_infos_.begin();
       i != _region_infos_.end(); i++) {
    const std::string& the_region_name = i->first;
    for (std::set<std::string>::const_iterator j =
             i->second.get_logical_ids().begin();
         j != i->second.get_logical_ids().end(); j++) {
      ...
      if (associated_logicals.count(log_id) > 0) {
        ...
        DT_COMMAND_RETURNED_ERROR(
            cri, datatools::command::CEC_FAILURE,
            "Logical volume '"
...
/*
// static
bool region_info::validate_for_geant4(const region_infos_dict_type &
region_infos_)
{
bool valid = true;
...
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(mctools::g4::region_info, ocd_) {
  ...
  ocd_.set_class_description(
      "The description of a Geant4 region attached to the virtual geometry");
  ...
  ocd_.set_class_documentation(
      ...
      "      \n"
      "A region consists in a set of logical volumes that are considered to "
      "share    \n"
      ...
      "      \n"
      "specific behaviour of the tracking depending of the regions traversed "
      "by      \n"
      "tracked particles: set special cuts on steps, tracks, generation of "
      "secondary \n"
      "particles, set special electromagnetic fields or processes...).         "
      "      \n"
      "Practically a region is explicitely defined by a set of logical volumes "
      "in    \n"
      "the setup. However the present class is equipped with a high-level "
      "interface  \n"
      "which allows users to define a region from various useful criteria:     "
      "      \n"
      "                                                                        "
      "      \n"
      " * Explicit list of logical volumes (given by names) the user must "
      "manually   \n"
      "   specify the list of volumes to be added in the region.               "
      "      \n"
      " * Explicit list of materials (given by names) of which the volumes in "
      "the    \n"
      "   region are made of.                                                  "
      "      \n"
      " * Explicit state of the material of which the volumes in the region "
      "are made \n"
      "   of. Supported values are: ``\"solid\"``, ``\"liquid\"``, "
      "``\"gas\"``.      \n"
      " * Explicit density range of the material of which the volumes in the "
      "region  \n"
      "   are made of.                                                         "
      "      \n"
      "                                                                        "
      "      \n"
      "Given one of the criteria above to build a region, it is still possible "
      "to    \n"
      "explicitely remove some extra logical volumes that turns to be excluded "
      "from  \n"
      "a region.                                                               "
      "      \n"
      "                                                                        "
      "      \n"
      "Note that the top-level *world* volume must not be set to any user "
      "region.    \n"
...
DOCD_CLASS_SYSTEM_REGISTRATION(mctools::g4::region_info,
                               "mctools::g4::region_info")
