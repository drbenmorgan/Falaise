...
/*
void em_field_equation_of_motion::initialize(const datatools::properties &
config_)
{
  return;
}
...
double em_field_equation_of_motion::get_anomaly() const { return _anomaly_; }
...
bool em_field_equation_of_motion::is_with_spin() const { return _with_spin_; }

void em_field_equation_of_motion::SetChargeMomentumMass(
    G4double particle_charge_,  // in e+ units
    G4double particle_momentum_, G4double particle_mass_) {
  DT_LOG_TRACE_ENTERING(_logprio());

  DT_LOG_DEBUG(_logprio(), " -> Particle charge = "
                               << particle_charge_ / CLHEP::eplus << "e ");
...
void em_field_equation_of_motion::EvaluateRhsGivenB(const G4double y_[],
  ...
  if (_charge_ != 0.0) {
    dydx_[PX] =
        coef1 * (y_[PY] * field_[EMFIELD_BZ] - y_[PZ] * field_[EMFIELD_BY]);
    dydx_[PY] =
        coef1 * (y_[PZ] * field_[EMFIELD_BX] - y_[PX] * field_[EMFIELD_BZ]);
    dydx_[PZ] =
        coef1 * (y_[PX] * field_[EMFIELD_BY] - y_[PY] * field_[EMFIELD_BX]);
    if (!_only_magnetic_) {
  ...
  if (is_with_spin()) {
    // The BMT equation, following J.D.Jackson, Classical Electrodynamics,
    // Second Edition, dS/dt = (e/mc) S \cross
    //              [ (g/2-1 +1/\gamma) B
