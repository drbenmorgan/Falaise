...
const run_action& event_action::get_run_action() const { return *_run_action_; }

run_action& event_action::grab_run_action() { return *_run_action_; }
...
void event_action::set_external_event_data(
    ::mctools::simulated_data& a_external_event_data) {
  _external_event_data_ = &a_external_event_data;
  return;
}

bool event_action::is_initialized() const { return _initialized_; }
...
void event_action::_at_init_() { return; }

void event_action::_at_reset_() { return; }
...
void event_action::initialize(const datatools::properties& config_) {
  ...
  if (config_.has_key("event_model.hit_collection_type")) {
    ...
    if (event_model_collection_type == "plain") {
      event_data.set_collection_type(
          ::mctools::simulated_data::PLAIN_HIT_COLLECTION_TYPE);
    } else if (event_model_collection_type == "handle") {
      event_data.set_collection_type(
          ::mctools::simulated_data::HANDLE_HIT_COLLECTION_TYPE);
    } else {
  ...
  if (config_.has_key("save_only_tracked_events")) {
    _save_only_tracked_events_ =
        config_.fetch_boolean("save_only_tracked_events");
  }
...
bool event_action::is_aborted_event() const { return _aborted_event_; }
...
bool event_action::is_killed_event() const { return _killed_event_; }
...
void event_action::BeginOfEventAction(const G4Event* event_) {
  ...
  if (_run_action_->has_number_of_events_modulo()) {
    ...
    if (!print_it &&
        _run_action_->get_manager().get_number_of_events() !=
            manager::NO_LIMIT &&
        event_id ==
            (int)_run_action_->get_manager().get_number_of_events() - 1) {
      print_it = true;
  ...
  if (is_aborted_event()) {
    ...
    DT_LOG_WARNING(_logprio(),
                   "Event #" << event_id << " is aborted. Exiting.");
...
void event_action::EndOfEventAction(const G4Event* event_) {
  ...
  if (is_killed_event()) {
    DT_LOG_DEBUG(_logprio(),
                 "Event #" << event_id << " is killed and thus not tracked!");
  ...
  bool save_this_event =
      !_run_action_->get_manager().dont_save_no_sensitive_hit_events();
...
void event_action::_mt_control_() {
  ...
  if (_run_action_->get_manager().has_simulation_ctrl()) {
    ...
    {
      DT_LOG_TRACE(_logprio(),
                   "Wait for the release of the event control by the external "
                   "process...");
      boost::mutex::scoped_lock lock(*SimCtrl.event_mutex);
      while (SimCtrl.event_availability_status ==
...
void event_action::_process_sensitive_hits_(const G4Event* event_,
                                            bool& save_this_event_) {
  ...
  for (int i = 0; i < (int)HCE->GetCapacity(); i++) {
    ...
    if (hc != 0) {
      DT_LOG_DEBUG(_logprio(), "Hit collection '"
                                   << hc->GetName()
  ...
  for (detector_construction::sensitive_detector_dict_type::const_iterator iSD =
       ...
       iSD != _detector_construction_->get_sensitive_detectors().end(); iSD++) {
    ...
    G4VHitsCollection* the_hits_collection =
        HCE->GetHC(the_detector.get_HCID());
    if (the_hits_collection == 0) {
      DT_LOG_DEBUG(_logprio(), "No hit to process from sensitive detector '"
                                   << sensitive_category << "'...");
      continue;
    }
     ...
     * are conventionally considered as 'private' hit categories, added by the
     * user for special purpose (debug, visualization...). 'official' MC hit
     * categories should never starts with "__" and should be associated to some
     * sensitive detector.
     */
    const bool is_private_category =
        boost::starts_with(sensitive_category, "__");
    ...
    if (the_detector.get_hit_processors().size() > 0) {
      ...
      for (sensitive_detector::hit_processor_dict_type::iterator iproc =
           ...
           iproc != the_detector.grab_hit_processors().end(); iproc++) {
        /*
  ...
  if (public_sensitive_category_counter > 0) {
    DT_LOG_DEBUG(_logprio(),
                 "Tagging current event with 'physical' sensitive hits "
...
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(mctools::g4::event_action, ocd_) {
  ...
  ocd_.set_class_documentation(
      "This is Geant4 simulation engine embedded event action.      \n");
...
DOCD_CLASS_SYSTEM_REGISTRATION(mctools::g4::event_action,
                               "mctools::g4::event_action")
