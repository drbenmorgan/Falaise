...
DPP_MODULE_REGISTRATION_IMPLEMENT(simulation_module,
...
// Initialization :
void simulation_module::initialize(
  ...
  if (geant4Parameters_.logging == "fatal") {
    if (config_.has_flag("manager.logging.priority")) {
      geant4Parameters_.logging =
          config_.fetch_string("manager.logging.priority");
  ...
  if (geometryManagerRef_ == 0) {
    ...
    if (service_manager_.has(geometryServiceName_) &&
        service_manager_.is_a<geomtools::geometry_service>(
            geometryServiceName_)) {
      // Fetch a reference to the geometry service :
      geomtools::geometry_service& Geo =
          service_manager_.grab<geomtools::geometry_service>(
              geometryServiceName_);
      ...
      DT_THROW_IF(true, std::logic_error,
                  "Module '" << get_name() << "' has no '"
                             << geometryServiceName_ << "' geometry service !");
    }
  }

  DT_THROW_IF(geometryManagerRef_ == 0, std::logic_error,
              "Missing geometry manager !");

  if (simdataBankName_.empty()) {
    // Use the default label for the 'simulated data' bank :
    simdataBankName_ =
        ::mctools::event_utils::EVENT_DEFAULT_SIMULATED_DATA_LABEL;  // "SD"
  }
...
// Processing :
dpp::base_module::process_status simulation_module::process(
  ...
  return status == 0 ? dpp::base_module::PROCESS_OK
                     : dpp::base_module::PROCESS_FATAL;
}
...
void simulation_module::set_geometry_manager(
...
void simulation_module::set_geant4_parameters(
    const manager_parameters& params_) {
  DT_THROW_IF(is_initialized(), std::logic_error,
              "Module '" << get_name() << "' is already initialized !");
  ...
  {
    {
      ...
      DT_LOG_TRACE(get_logging_priority(),
                   "Notify that event control is now available for the G4 "
                   "simulation thread...");
    ...
    {
      DT_LOG_TRACE(get_logging_priority(),
                   "Wait for the release of the event control by the G4 "
                   "simulation thread...");
      boost::mutex::scoped_lock lock(*geant4SimulationController_->event_mutex);
