...
class em_field_g4_stuff : public datatools::i_tree_dumpable,
 ...
 private:
  ...
  electromagnetic_field*
      _eb_field_;  //!< Electromagnetic field (referenced, not deleted)
  ...
  double _delta_one_step_;      //!< Position error which is acceptable in an
                                //!integration step (same order of magnitude as
                                //!_delta_intersection_)
  double _delta_intersection_;  //!< Upper limit inaccuracy of a single boundary
                                //!crossing (same order of magnitude as
                                //!_delta_one_step_)
  double _eps_min_;             //!< Lower limit on the relative error of the
                                //!position/momentum inaccuracy
  double _eps_max_;             //!< Upper limit on the relative error of the
                                //!position/momentum inaccuracy
  bool _spin_;                  //!< Spin flag
  bool _propagate_to_daughters_;  //!< Flag to propagate the field to all
                                  //!daughter volumes

  G4EquationOfMotion*
      _equation_;  //!< Equation of motion (deleted by the chord finder!)
  G4PropagatorInField*
      _field_propagator_;  //!< Field propagator   (referenced, not deleted)
  G4MagIntegratorStepper*
      _field_stepper_;  //!< ODE stepper        (deleted by this object)
  G4MagInt_Driver* _integration_driver_;  //!< Integration driver (deleted by
                                          //!the chord finder!)
  G4ChordFinder*
      _chord_finder_;  //!< Chord finder       (deleted by this object)
  G4FieldManager*
      _field_manager_;  //!< Field manager      (deleted by this object)
};
