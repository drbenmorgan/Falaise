...
bool electromagnetic_field::has_name() const { return !_name_.empty(); }
...
const std::string& electromagnetic_field::get_name() const { return _name_; }
...
bool electromagnetic_field::is_initialized() const { return _initialized_; }

bool electromagnetic_field::has_field() const { return _field_ != 0; }

void electromagnetic_field::set_field(
    const emfield::base_electromagnetic_field& f_) {
  _field_ = &f_;
  return;
}

const emfield::base_electromagnetic_field& electromagnetic_field::get_field()
    const {
  DT_THROW_IF(!has_field(), std::logic_error, "No electromagnetic field !");
  return *_field_;
}

bool electromagnetic_field::is_active() const { return has_field(); }
...
void electromagnetic_field::reset() {
  DT_THROW_IF(!_initialized_, std::logic_error,
              "Geant4 electromagnetic field '" << get_name()
                                               << "' is not initialized !");
    ...
    if (config_.has_key("mode")) {
      ...
      if (mode_str == "standalone_magnetic_field") {
        ...
        DT_THROW(std::logic_error,
                 "Missing 'mode' property in Geant4 electromagnetic field '"
                     << get_name() << "' !");
      }
      ...
      if (mode == STANDALONE_ELECTRIC_FIELD) {
        if (config_.has_key("electric_field_unit")) {
          ...
          electric_field_unit =
              datatools::units::get_electric_field_unit_from(efu);
        }
        ...
        if (!geomtools::is_valid(_standalone_constant_electric_field_)) {
          if (config_.has_key("standalone_constant_electric_field")) {
            ...
            DT_THROW_IF(seg.size() != 3, std::logic_error,
                        "Invalid dimension for vector of constant electric "
                        "field coordinates in Geant4 electric field '"
                            << get_name() << "' !");
            _standalone_constant_electric_field_.set(seg[0], seg[1], seg[2]);
            if (!config_.has_explicit_unit(
                    "standalone_constant_electric_field")) {
              _standalone_constant_electric_field_ *= electric_field_unit;
              DT_LOG_NOTICE(_logprio(),
                            "Forcing unit for standalone constant electric "
                            "field in Geant4 electric field '"
                                << get_name() << "'...");
            }
          }
        }
      ...
      if (mode == STANDALONE_MAGNETIC_FIELD) {
        if (config_.has_key("magnetic_field_unit")) {
          ...
          magnetic_field_unit =
              datatools::units::get_magnetic_field_unit_from(mfu);
        }
        ...
        if (!geomtools::is_valid(_standalone_constant_mag_field_)) {
          if (config_.has_key("standalone_constant_mag_field")) {
            ...
            DT_THROW_IF(smg.size() != 3, std::logic_error,
                        "Invalid dimension for vector of constant magnetic "
                        "field coordinates in Geant4 magnetic field '"
                            << get_name() << "' !");
            ...
            if (!config_.has_explicit_unit("standalone_constant_mag_field")) {
              ...
              DT_LOG_NOTICE(_logprio(),
                            "Forcing unit for standalone constant magnetic "
                            "field in Geant4 magnetic field '"
                                << get_name() << "'...");
            }
          }
        }
      ...
      /*
        ...
        config_.fetch_string("electromagnetic_field.geom_plugin");
        }
        else {
        std::ostringstream message;
        message << "mctools::g4::electromagnetic_field::initialize: "
        << "Missing field plugin name property '" <<
        "electromagnetic_field.geom_plugin" << "' !"; throw
...
void electromagnetic_field::GetFieldValue(const double position_[4],
  ...
  DT_LOG_TRACE(_logprio(),
               "Entering GetFieldValue for Geant4 electromagnetic field '"
                   << get_name() << "'...");
  DT_THROW_IF(!_initialized_, std::logic_error,
              "Geant4 electromagnetic field '" << get_name()
                                               << "' is not initialized !");
  ...
  if (_field_ != 0) {
    ...
    if (_field_check_pos_time_) {
      DT_THROW_IF(!_field_->position_and_time_are_valid(pos, time),
                  ...
                  "Position and time at "
                      ...
                      << " are not valid for electromagnetic field named '"
                      << _name_ << "' !");
    }
    if (_field_->is_magnetic_field()) {
      ...
      int status =
          _field_->compute_magnetic_field(pos, time, the_magnetic_field);
      DT_THROW_IF(status != 0, std::logic_error,
                  "Magnetic field named '"
                      << _name_ << "' cannot compute magnetic field value at "
                      << pos / CLHEP::mm << " [mm] / "
                      << time / CLHEP::nanosecond << " [ns] !");
      DT_LOG_TRACE(_logprio(), "Geant4 magnetic field '"
                                   << get_name() << "' is : "
                                   << the_magnetic_field / CLHEP::gauss
                                   << " gauss");
    ...
    if (_field_->is_electric_field()) {
      ...
      int status =
          _field_->compute_electric_field(pos, time, the_electric_field);
      DT_THROW_IF(status != 0, std::logic_error,
                  "Electric field named '"
                      << _name_ << "' cannot compute electric field value at "
                      << pos / CLHEP::mm << " [mm] / "
                      << time / CLHEP::nanosecond << " [ns] !");
      DT_LOG_TRACE(_logprio(),
                   "Geant4 electric field '"
                       << get_name() << "' is : "
                       << the_electric_field / (CLHEP::volt / CLHEP::meter)
                       << " V/m");
  ...
  DT_LOG_TRACE(_logprio(),
               "Exiting GetFieldValue for Geant4 electromagnetic field '"
                   << get_name() << "'.");
  return;
}
...
void electromagnetic_field::dump(std::ostream& out_) const {
  ...
  if (geomtools::is_valid(_standalone_constant_mag_field_)) {
    out_ << "|-- Standalone constant magnetic field : "
         << _standalone_constant_mag_field_ / CLHEP::gauss << " gauss"
         << std::endl;
  } else if (geomtools::is_valid(_standalone_constant_electric_field_)) {
  ...
  out_ << "`-- Check field pos/time   : " << _field_check_pos_time_
       << std::endl;
  return;
}
