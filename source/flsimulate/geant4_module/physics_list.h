...
class physics_list : public G4VModularPhysicsList,
 ...
 public:
  ...
  const base_physics_constructor& get_physics_constructor(
 ...
 private:
  ...
  // Production cuts for secondary particles (only gamma, electron, positron,
  // proton):
  bool
      _using_production_cuts_;  //!< Flag to activate the use of production cuts
  double _production_cuts_low_energy_;            //!< Low edge energy for production cuts
                                                  //!(in unit of energy)
  double _production_cuts_high_energy_;           //!< High edge energy for production
                                                  //!cuts (in unit of energy)
  double _production_cuts_default_value_;         //!< Default value for production
                                                  //!cuts (in unit of length)
  production_cuts_info _production_cuts_values_;  //!< Default production cuts
                                                  //!for gammas, electron,
                                                  //!positron and protons
  std::map<std::string, production_cuts_info>
      _production_cuts_per_region_;  //!< Map region and specific production
                                     //!cuts
  ...
  base_physics_constructor::factory_register_type
      _factory_register_;                                //!< The embedded factory register for physics
                                                         //!constructors
  physics_constructor_dict_type _physics_constructors_;  //!< The embedded
                                                         //!dictionnary of
