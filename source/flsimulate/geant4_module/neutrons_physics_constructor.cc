...
/*
  ...
  neutrons:
  Bertini cascade (0 – 9.9 GeV), GHEISHA (9.5 – 25 GeV), QGSP (12 GeV to 100
  TeV) HadronElastic (all energies) GHEISHA capture (all energies) GHEISHA
  fission (all energies)
  ...
  Identical to QGSP_BERT, except high precision neutron (NeutronHP) models used
  for neutron elastic, inelastic, capture and fission below energies of 20 MeV
*/
...
void neutrons_physics_constructor::initialize(
  ...
  if (config_.has_key("neutrons.inelastic")) {
    _use_neutrons_inelastic_process_ =
        config_.fetch_boolean("neutrons.inelastic");
  }
  DT_LOG_DEBUG(_logprio(), "Use neutrons inelastic process: "
                               << _use_neutrons_inelastic_process_);

  if (config_.has_key("neutrons.elastic")) {
    _use_neutrons_elastic_process_ = config_.fetch_boolean("neutrons.elastic");
  }
  DT_LOG_DEBUG(_logprio(), "Use neutrons elastic process: "
                               << _use_neutrons_elastic_process_);
  ...
  if (config_.has_key("protons.inelastic")) {
    _use_protons_inelastic_process_ =
        config_.fetch_boolean("protons.inelastic");
  }
  DT_LOG_DEBUG(_logprio(), "Use protons inelastic process: "
                               << _use_protons_inelastic_process_);

  if (config_.has_key("protons.elastic")) {
    _use_protons_elastic_process_ = config_.fetch_boolean("protons.elastic");
  }
  DT_LOG_DEBUG(_logprio(), "Use protons elastic process: "
                               << _use_protons_elastic_process_);

  // Electron processes :
  if (config_.has_key("electrons.hadronic")) {
    _use_electrons_hadronic_process_ =
        config_.fetch_boolean("electrons.hadronic");
  }
  DT_LOG_DEBUG(_logprio(), "Use electrons-hadrons process: "
                               << _use_electrons_hadronic_process_);
  ...
  DT_LOG_DEBUG(_logprio(),
               "Use gamma-hadrons process: " << _use_gammas_hadronic_process_);
...
void neutrons_physics_constructor::tree_dump(std::ostream& out_,
  ...
  out_ << indent << datatools::i_tree_dumpable::tag
       << "Use high-energy process        : " << (_use_HE_ ? "Yes" : "No")
       << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag
  ...
  while ((*theParticleIterator)()) {
    ...
    if (pname == "neutron") {
      ...
      if (_use_neutrons_inelastic_process_) {
        ...
        G4LENeutronInelastic* the_intermediate_energy_neutron_inelastic_model =
            0;
        if (_use_HE_) {
          the_intermediate_energy_neutron_inelastic_model =
              new G4LENeutronInelastic();
          the_intermediate_energy_neutron_inelastic_model->SetMinEnergy(
              9.5 * CLHEP::GeV);
          the_intermediate_energy_neutron_inelastic_model->SetMaxEnergy(
              25. * CLHEP::GeV);
        }
        ...
        the_neutron_inelastic_process->RegisterMe(
            the_low_energy_neutron_inelastic_model);
        if (_use_HE_)
          the_neutron_inelastic_process->RegisterMe(
              the_intermediate_energy_neutron_inelastic_model);
      ...
      if (_use_neutrons_elastic_process_) {
        ...
        if (_use_HE_) {
          ...
          the_high_energy_neutron_elastic_model->SetMinEnergy(19.5 *
                                                              CLHEP::MeV);
        ...
        the_neutron_elastic_process->RegisterMe(
            the_neutron_thermal_elastic_model);
        if (_use_HE_)
          the_neutron_elastic_process->RegisterMe(
              the_high_energy_neutron_elastic_model);
    ...
    if (pname == "proton") {
      ...
      if (_use_protons_inelastic_process_) {
        ...
        if (_use_HE_) {
          ...
          the_high_energy_proton_inelastic_model->SetMinEnergy(9.5 *
                                                               CLHEP::GeV);
          the_high_energy_proton_inelastic_model->SetMaxEnergy(25. *
                                                               CLHEP::GeV);
          QGSP_model->SetMinEnergy(11.9 * CLHEP::GeV);
        }
        ...
        if (_use_HE_)
          the_proton_inelastic_process->RegisterMe(
              the_high_energy_proton_inelastic_model);
    ...
    if (pname == "e-") {
      if (_use_electrons_hadronic_process_) {
        ...
        the_electron_inelastic_process->RegisterMe(
            the_electron_nuclear_reaction);
...
// static
void neutrons_physics_constructor::init_ocd(
    datatools::object_configuration_description& ocd_) {
  // Inherits configuration properties from its base class:
  // ::mctools::g4::base_physics_constructor::init_ocd(ocd_);

  {
    // Description of the 'neutrons.use_high_energy_process' configuration
    // property :
    datatools::configuration_property_description& cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern("neutrons.high_energy")
        .set_terse_description(
            "Flag to activate high-energy process for neutrons and protons")
  ...
  {
    // Description of the 'neutrons.use_neutrons_inelastic_process'
    // configuration property :
    datatools::configuration_property_description& cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern("neutrons.inelastic")
        .set_terse_description(
            "Flag to activate inelastic process for neutrons")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
  ...
  {
    // Description of the 'neutrons.use_neutrons_elastic_process' configuration
    // property :
    datatools::configuration_property_description& cpd =
        ocd_.add_property_info();
  ...
  {
    // Description of the 'neutrons.use_protons_inelastic_process' configuration
    // property :
    datatools::configuration_property_description& cpd =
        ocd_.add_property_info();
  ...
  {
    // Description of the 'neutrons.use_protons_elastic_process' configuration
    // property :
    datatools::configuration_property_description& cpd =
        ocd_.add_property_info();
  ...
  {
    // Description of the 'neutrons.use_electrons_hadrons_process' configuration
    // property :
    datatools::configuration_property_description& cpd =
        ocd_.add_property_info();
  ...
  {
    // Description of the 'neutrons.use_gamma_hadrons_process' configuration
    // property :
    datatools::configuration_property_description& cpd =
        ocd_.add_property_info();
...
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(::mctools::g4::neutrons_physics_constructor,
                                ocd_) {
