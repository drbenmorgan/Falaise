...
class region_info : public datatools::enriched_base {
 public:
  ...
  void add_logical_volume(const std::string &logical_id_,
 ...
 private:
  ...
  std::set<std::string> _logical_ids_;  ///< Set of logical volumes' names
...
class regions_setup : public datatools::i_tree_dumpable {
 public:
  ...
  // Validate for Geant4...
  // static bool validate_for_geant4(const region_infos_dict_type &
  // region_infos_);
 ...
 private:
  const geomtools::manager
      *_geom_manager_;  ///< Handle to an external geometry manager
