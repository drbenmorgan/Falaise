...
void manager::reset() {
  // std::cerr << "DEVEL: mctools::g4::manager::reset: Entering..." <<
  // std::endl;
...
bool manager::is_automatic() const { return is_batch() && _g4_macro_.empty(); }
...
void manager::set_service_manager(datatools::service_manager& sm) {
  DT_THROW_IF(_initialized_, std::logic_error,
              "Operation prohibited ! Manager is locked !");
...
const genbb::manager& manager::get_eg_manager() const { return _eg_manager_; }

genbb::manager& manager::grab_eg_manager() { return _eg_manager_; }

bool manager::has_event_generator() const { return _event_generator_ != 0; }

const genbb::i_genbb& manager::get_event_generator() const {
  DT_THROW_IF(!this->has_event_generator(), std::logic_error,
...
const genvtx::manager& manager::get_vg_manager() const { return _vg_manager_; }

genvtx::manager& manager::grab_vg_manager() { return _vg_manager_; }

bool manager::has_vertex_generator() const { return _vertex_generator_ != 0; }
...
uint32_t manager::get_number_of_events() const { return _number_of_events_; }
...
void manager::set_number_of_events(uint32_t n) {
  ...
  if (_number_of_events_ > NUMBER_OF_EVENTS_WARNING_LIMIT &&
    ...
    DT_LOG_WARNING(_logprio(),
                   "Number of events is high = " << _number_of_events_);
  }
...
void manager::set_number_of_events_modulo(int a_m) {
  ...
  DT_LOG_DEBUG(_logprio(),
               "Number of events modulo = " << _number_of_events_modulo_);
}
...
bool manager::is_batch() const { return !is_interactive(); }

bool manager::is_interactive() const { return _interactive_; }
...
bool manager::has_g4_macro() const { return !_g4_macro_.empty(); }

const std::string& manager::get_g4_macro() const { return _g4_macro_; }
...
bool manager::has_track_history() const { return _use_track_history_; }

const track_history& manager::get_track_history() const {
  return _track_history_;
}

track_history& manager::grab_track_history() { return _track_history_; }

bool manager::forbids_private_hits() const { return _forbid_private_hits_; }
...
//----------------------------------------------------------------------
bool manager::has_simulation_ctrl() const { return _simulation_ctrl_ != 0; }

void manager::set_simulation_ctrl(simulation_ctrl& a_simulation_ctrl) {
  DT_THROW_IF(_initialized_, std::logic_error,
              "Operation prohibited ! Manager is locked !");
  DT_THROW_IF(has_simulation_ctrl(), std::logic_error,
              "Operation prohibited ! Manager already got a 'simulation ctrl' "
              "object !");
...
//----------------------------------------------------------------------
mygsl::rng& manager::grab_vg_prng() { return _vg_prng_; }

mygsl::rng& manager::grab_eg_prng() { return _eg_prng_; }

mygsl::rng& manager::grab_shpf_prng() { return _shpf_prng_; }

const mygsl::rng& manager::get_mgr_prng() const { return _mgr_prng_; }

mygsl::rng& manager::grab_mgr_prng() { return _mgr_prng_; }

const g4_prng& manager::get_g4_prng() const { return _g4_prng_; }

g4_prng& manager::grab_g4_prng() { return _g4_prng_; }

const mygsl::seed_manager& manager::get_seed_manager() const {
  return _seed_manager_;
}

mygsl::seed_manager& manager::grab_seed_manager() { return _seed_manager_; }
...
void manager::record_current_prng_states() {
  // G4 PRNG :
  _mgr_prng_.to_buffer(
      _prng_state_manager_.get_state(g4_manager_label()).state_buffer);
  ...
  _eg_prng_.to_buffer(
      _prng_state_manager_.get_state(event_generator_label()).state_buffer);

  // PRNG of the Step Hit Processor Factory :
  _shpf_prng_.to_buffer(
      _prng_state_manager_.get_state(shpf_label()).state_buffer);
...
int manager::get_mgr_prng_seed() const { return _mgr_prng_seed_; }
  ...
  if (_output_data_format_ == io_utils::DATA_FORMAT_BANK) {
    if (_output_data_bank_label_.empty()) {
      set_output_data_bank_label(
          event_utils::event_default_simulated_data_label());
    }
  }
}
...
void manager::set_output_data_bank_label(const std::string& bl_) {
  ...
  DT_THROW_IF(
      _output_data_format_ != io_utils::DATA_FORMAT_BANK, std::logic_error,
...
bool manager::using_time_stat() const { return _use_time_stat_; }
...
const manager::CT_map& manager::get_CT_map() const { return _CTs_; }

manager::CT_map& manager::grab_CT_map() { return _CTs_; }

//----------------------------------------------------------------------
// Output Profiles
//----------------------------------------------------------------------
bool manager::has_supported_output_profile(
    const std::string& profile_id_) const {
...
void manager::add_supported_output_profile(const std::string& profile_id_,
  ...
  DT_THROW_IF(profile_id.empty(), std::logic_error,
  ...
  DT_THROW_IF(profile_id.find('+') != profile_id.npos, std::logic_error,
              "Forbidden '+' character in the output profile Id '" << profile_id
                                                                   << "' !");
  DT_THROW_IF(profile_id.find(' ') != profile_id.npos, std::logic_error,
              "Forbidden space character in the output profile Id '"
                  << profile_id << "' !");
  DT_THROW_IF(
      has_supported_output_profile(profile_id), std::logic_error,
...
void manager::fetch_activated_output_profile_ids(
...
void manager::dump_base(std::ostream& os, const std::string& title,
  ...
  for (auto i = _supported_output_profile_ids_.begin();
       i != _supported_output_profile_ids_.end(); ++i) {
    auto j = i;
    j++;
    os << "|   ";
  ...
  for (auto i = _activated_output_profile_ids_.begin();
       i != _activated_output_profile_ids_.end(); ++i) {
    auto j = i;
    j++;
    os << "|   ";
...
void manager::initialize_impl() {
  ...
  if (has_g4_visualization()) {
    std::cerr << "DEVEL: manager::initialize_impl: new G4VisExecutive !!!"
              << std::endl;
...
void manager::run_simulation_impl() {
  ...
  if (has_g4_macro()) {
    ...
    if (!boost::filesystem::exists(g4_macro)) {
      ...
      G4Exception("mctools::g4::manager::_at_run_simulation", "FileError",
...

  {
    // 2011-03-03 FM :
    DT_LOG_NOTICE(_logprio(),
                  "Record the final PRNG internal states after the session has "
                  "stopped normaly.");
...
void manager::_init_core() {
  ...
  // Load a file with the content of the embedded 'seed_manager' for PRNGs
  // seeding:
  if (has_input_prng_seeds_file()) {
    DT_LOG_NOTICE(_logprio(), "PRNG seed dictionary from '"
    ...
    if (has_output_prng_seeds_file()) {
      ...
      DT_THROW_IF(
          input_prng_seeds_file == test_output_prng_seeds_file,
          std::logic_error,
    ...
    if (boost::filesystem::exists(input_prng_seeds_file)) {
      ...
      DT_THROW(std::runtime_error, "No PRNG seed dictionary input file '"
                                       << input_prng_seeds_file
                                       << "' to be loaded.");
    }
  }
  ...
  if (!dont_save_no_sensitive_hit_events()) {
    ...
    if (manager_config.has_key("dont_save_no_sensitive_hit_events")) {
      bool dont_save =
          manager_config.fetch_boolean("dont_save_no_sensitive_hit_events");
  ...
  if (get_init_seed_method() == "current_time") {
    ...
    DT_THROW(std::logic_error, "Initialization method '"
  ...
  // 2011-02-26 FM : only search for the 'g4_macro' property if '_g4_macro_' is
  // not set yet :
  if (!_g4_macro_.empty()) {
  ...
  if (_g4_tracking_verbosity_ == 0) {
    if (manager_config.has_key("g4_tracking_verbosity")) {
      int tracking_verbosity =
          manager_config.fetch_integer("g4_tracking_verbosity");
...
void manager::_init_geometry() {
  ...
  if (has_external_geom_manager()) {
    ...
    std::string geom_mgr_prop_filename =
        geometry_config.fetch_string("manager.config");
...
void manager::_init_event_generator() {
  ...
  primary_generator_config =
      _multi_config_->get("event_generator").get_properties();
  ...
  DT_THROW_IF(
      !_eg_manager_.has(eg_name), std::logic_error,
...
void manager::_init_detector_construction() {
  ...
  _user_detector_construction_->grab_step_hit_processor_factory()
      .set_external_prng(_shpf_prng_);
...
void manager::_init_run_action() {
  ...
  if (has_number_of_events_modulo()) {
    _user_run_action_->set_number_of_events_modulo(
...
void manager::_init_event_action() {
  ...
  _user_event_action_ =
      new event_action(*_user_run_action_, *_user_detector_construction_);
...
void manager::_init_seeds() {
  ...
  if (_seed_manager_.has_seed(shpf_label())) {
    ...
    DT_LOG_NOTICE(_logprio(), "Using registered seed for '" << shpf_label()
                                                            << "' : " << seed);
  }

  // Checks:
  DT_THROW_IF(!mygsl::seed_manager::seed_is_valid(_vg_prng_seed_),
              std::logic_error, "Invalid vertex generator seed value !");
  DT_THROW_IF(!mygsl::seed_manager::seed_is_valid(_eg_prng_seed_),
              std::logic_error, "Invalid event generator seed value !");
  DT_THROW_IF(!mygsl::seed_manager::seed_is_valid(_mgr_prng_seed_),
              std::logic_error, "Invalid G4 generator seed value !");
  DT_THROW_IF(!mygsl::seed_manager::seed_is_valid(_shpf_prng_seed_),
              std::logic_error, "Invalid SHPF generator seed value !");
...
void manager::_init_prngs_states() {
  ...
  if (has_input_prng_states_file()) {
    ...
    // Event generator PRNG :
    _eg_prng_.from_buffer(
        _prng_state_manager_.get_state(event_generator_label()).state_buffer);

    // PRNG for Step Hit Processors :
    _shpf_prng_.from_buffer(
        _prng_state_manager_.get_state(shpf_label()).state_buffer);
  }
}
...
void manager::_init_prngs() {
  ...
  //     DT_LOG_NOTICE(_logprio(), "Random Number Generator internal state was
  //     restored from file '"
  //                   << rsf << "'...");
  //     needs_seed = false;
  //     // if (_mgr_prng_seed_ != mygsl::random_utils::SEED_INVALID) {
  //     //   DT_LOG_NOTICE(_logprio(),
  //     //                 "Ignoring the seed for the Random Number Generator
  //     initialization !");
  //     // }
  //     // _mgr_prng_seed_ = mygsl::random_utils::SEED_INVALID;
  //   } else {
  //     DT_THROW(std::logic_error,
  //              "Random Number Generator initial state file '" << rsf.c_str()
  //              << "' does not exist !");
  //   }
  // }
  ...
  */
  ...
  DT_LOG_NOTICE(_logprio(), "Setting the HepRandomEngine to '"
                                << _g4_prng_.name() << "'...");
}
...
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(mctools::g4::manager, ocd_) {
...

  {
    ...
    cpd.set_name_pattern("output_profiles")
        ...
        .set_long_description(
            ...
            "      \n"
            "through the "
            "``mctools::g4::manager::add_supported_output_profile(...)`` \n"
            "method.                                                           "
            "      \n")
        .add_example(
  ...
  {
    // Description of the 'output_profiles.supported.${id}.description'
...

  {
    ...
    cpd.set_name_pattern("output_profiles.activation")
        ...
        .set_long_description(
            ...
            "            \n"
            "output profiles have already been activated in the simulation "
            "manager,        \n"
            "through the "
            "``mctools::g4::manager::activate_output_profile(...)`` or the     "
            "\n"
            "``mctools::g4::manager::apply_output_profiles_activation_rule(...)"
            "`` methods. \n")
        .add_example(
            ...
            "        \"muons_details\"  \\                                   \n"
            "        \"tracker_details\"                                     \n"
            "  output_profiles.activation: string = \"ecalo_details + "
            "muons_details\" \n");
  }

  {
    // Description of the 'dont_save_no_sensitive_hit_events' configuration
    // property :
    datatools::configuration_property_description& cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern("dont_save_no_sensitive_hit_events")
        ...
        .set_terse_description(
            "Flag used to record only events with sensitive hits")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
        // .set_long_description("This property is not taken into account if the
        // \n"
        //                       "*number of events* attributes has been set
        //                       previously through           \n" "the
        //                       ``mctools::g4::manager::set_number_of_events(...)``
        //                       method.         \n" "
        //                       \n"
        //                       )
        .add_example(
...

  {
    ...
    cpd.set_name_pattern("manager.config")
        ...
        .set_long_description(
            ...
            "      \n"
            "the ``mctools::g4::manager::set_external_geom_manager(...)`` "
            "method.    \n"
            "                                                                  "
            "      \n");
  }
  ...
  {
    // Description of the vertex generator 'manager.config' configuration
    // property :
    datatools::configuration_property_description& cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern("manager.config")
        ...
        .set_long_description(
            ...
            "            \n"
            "  manager.config : string as path = "
            "\"config/vertex_generator/manager.conf\"  \n"
            "                                                                  "
            "            \n");
  }

  {
    // Description of the event generator 'manager.config' configuration
    // property :
    datatools::configuration_property_description& cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern("manager.config")
        ...
        .set_terse_description(
            "Event generator manager main configuration file")
        .set_traits(datatools::TYPE_STRING)
        .set_path(true)
        .set_mandatory(true)
        .set_long_description(
            ...
            "            \n"
            "  manager.config : string as path = "
            "\"config/event_generator/manager.conf\"   \n"
            "                                                                  "
            "            \n");
  }
