...
bool magnetic_field::has_name() const { return !_name_.empty(); }
...
const std::string& magnetic_field::get_name() const { return _name_; }
...
bool magnetic_field::is_initialized() const { return _initialized_; }

bool magnetic_field::has_field() const { return _field_ != 0; }

bool magnetic_field::has_mag_field() const { return _field_ != 0; }

void magnetic_field::set_mag_field(
    const emfield::base_electromagnetic_field& mf_) {
  _field_ = &mf_;
  return;
}
...
const emfield::base_electromagnetic_field& magnetic_field::get_field() const {
  DT_THROW_IF(!has_field(), std::logic_error, "No field !");
  return *_field_;
}

const emfield::base_electromagnetic_field& magnetic_field::get_mag_field()
    const {
  DT_THROW_IF(!has_mag_field(), std::logic_error, "No magnetic field !");
  return *_field_;
}

bool magnetic_field::is_active() const { return has_mag_field(); }
...
void magnetic_field::initialize(const datatools::properties& config_) {
  ...
  if (!has_field()) {
    ...
    if (config_.has_key("mode")) {
      ...
      DT_THROW_IF(true, std::logic_error,
                  "Missing 'mode' property in Geant4 magnetic field '"
                      << get_name() << "' !");
    }
      ...
      if (!geomtools::is_valid(_standalone_constant_field_)) {
        if (config_.has_key("standalone_constant_mag_field")) {
          ...
          DT_THROW_IF(smg.size() != 3, std::logic_error,
                      "Invalid dimension for vector of constant magnetic field "
                      "coordinates in Geant4 magnetic field '"
                          << get_name() << "' !");
    ...
    /*
      ...
      config_.fetch_string("magnetic_field.geom_plugin");
      }
      else {
      std::ostringstream message;
      message << "mctools::g4::magnetic_field::initialize: "
...
void magnetic_field::GetFieldValue(const double position_[4],
  ...
  DT_LOG_TRACE(_logprio(), "Entering GetFieldValue for Geant4 magnetic field '"
                               << get_name() << "'...");
  // DT_LOG_TRACE(datatools::logger::PRIO_ALWAYS, "Entering GetFieldValue for
  ...
  if (_field_ != 0) {
    DT_LOG_TRACE(_logprio(),
                 "Compute magnetic field for Geant4 magnetic field '"
                     << get_name() << "'...");
    // DT_LOG_TRACE(datatools::logger::PRIO_ALWAYS, "Compute magnetic field for
    // Geant4 magnetic field '" << get_name() << "'...");
    ...
    if (_field_check_pos_time_) {
      DT_THROW_IF(
          !_field_->position_and_time_are_valid(pos, time), std::logic_error,
          "Position and time at " << pos / CLHEP::mm << " [mm] / "
                                  << time / CLHEP::nanosecond << " [ns] "
                                  << " are not valid for magnetic field named '"
                                  << _name_ << "' !");
    }
    ...
    DT_THROW_IF(status != 0, std::logic_error,
                "Magnetic field named '"
                    ...
                    << pos / CLHEP::mm << " [mm] / " << time / CLHEP::nanosecond
                    << " [ns] !");
  ...
  DT_LOG_TRACE(_logprio(), "Exiting GetFieldValue for Geant4 magnetic field '"
                               << get_name() << "'.");
  return;
}
