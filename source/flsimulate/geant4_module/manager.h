...
class manager : public loggable_support {
 ...
 public:
  ...
  /// Set the method used to set the initialization seed for the generation of
  /// seeds
  ...
  const std::map<std::string, std::string>& get_supported_output_profiles()
      const;
  ...
  /// Basic print
  void dump_base(std::ostream& out_ = std::clog, const std::string& title_ = "",
 ...
 private:
  ...
  // Configuration:
  const datatools::multi_properties* _multi_config_ =
      nullptr;  //!< Setup parameters
  ...
  simulation_ctrl* _simulation_ctrl_ =
      nullptr;  //!< Simulation thread control instance

  std::map<std::string, std::string>
      _supported_output_profile_ids_;  //!< Supported simulation output profiles
  std::string _output_profiles_activation_rule_;         //!< Activation rule for
                                                         //!output profiles
  std::set<std::string> _activated_output_profile_ids_;  //!< Activated
                                                         //!simulation output
                                                         //!profile Ids
  ...
  // Geometry manager :
  const geomtools::manager* _external_geom_manager_ =
      nullptr;                        //!< External geometry manager
  ...
  genvtx::i_vertex_generator* _vertex_generator_ =
      nullptr;  //!< Active vertex generator
  ...
  int _shpf_prng_seed_;    //!< Seed for the embedded PRNG for step hit processor
                           //!factories
  ...
  // G4 objects:
  G4VSteppingVerbose* _g4_stepping_verbosity_ =
      nullptr;  //!< Geant4 stepping verbosity instance
  ...
  // A PRNG seed manager :
  std::string
      _init_seed_method_;  //!< Initialization method of the seed manager
  mygsl::prng_seed_manager _seed_manager_;  //!< PRNGs' seed manager

  // A PRNG state manager :
  mygsl::prng_state_manager
      _prng_state_manager_;      //!< Manager for PRNGs' internal states
  int _prng_state_save_modulo_;  //!< Event number modulo to backup PRNGs'
                                 //!internal states
  ...
  // User:
  std::string
      _input_prng_seeds_file_;  //!< Input filename for loading of PRNG seeds
  std::string
      _output_prng_seeds_file_;  //!< Output filename for storage of PRNG seeds
  std::string _input_prng_states_file_;             //!< Input filename for loading of PRNG
                                                    //!internal states
  std::string _output_prng_states_file_;            //!< Output filename for storage of
                                                    //!PRNG internal states
  io_utils::data_format_type _output_data_format_;  //!< The data format of the
                                                    //!output file ("plain" or
                                                    //!"bank")
  std::string _output_data_bank_label_;             //!< The label of the data bank used to
                                         //!store simulated data ("bank" format
                                         //!only)
  ...
  bool _forbid_private_hits_;     //!< Flag to disable the storage of MC true hits
                                  //!from 'private' collection of hits
  bool _dont_save_no_sensitive_hit_events_;  //!< Flag to store MC true hits
                                             //!from 'non-sensitive' volumes
