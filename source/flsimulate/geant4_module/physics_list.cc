...
void physics_list::production_cuts_info::initialize(
    double default_cut_gamma_, double default_cut_electron_,
...
const physics_constructor_dict_type& physics_list::get_physics_constructors()
    const {
  return _physics_constructors_;
}

physics_constructor_dict_type& physics_list::grab_physics_constructors() {
  return _physics_constructors_;
}
...
const base_physics_constructor& physics_list::get_physics_constructor(
    const std::string& pc_name_) {
  physics_constructor_dict_type::const_iterator pc_found =
      _physics_constructors_.find(pc_name_);
  DT_THROW_IF(
      pc_found == _physics_constructors_.end(), std::logic_error,
      "Cannot find physics constructor with name '" << pc_name_ << "' !");
  DT_THROW_IF(
      pc_found->second.handle, std::logic_error,
...
void physics_list::initialize(const datatools::properties& config_) {
  ...
  _factory_register_.import(DATATOOLS_FACTORY_GET_SYSTEM_REGISTER(
      mctools::g4::base_physics_constructor));
  ...
  if (config_.has_key("geant4.physics_list")) {
    ...
    DT_THROW_IF(!geant4_physics_list_factory.IsReferencePhysList(
                    _geant4_physics_list_name_),
                std::logic_error,
                "Geant4 physics list factory does not have any '"
                    << _geant4_physics_list_name_ << "' physics list !");
    _geant4_physics_list_.reset(
        geant4_physics_list_factory.GetReferencePhysList(
            _geant4_physics_list_name_));
  }
  ...
  if (!has_geant4_physics_list()) {
    ...
    DT_THROW_IF(physics_constructors_names.size() == 0, std::logic_error,
    ...
    for (size_t i = 0; i < physics_constructors_names.size(); i++) {
      ...
      DT_THROW_IF(!config_.has_key(pc_config_key.str()), std::logic_error,
                  "Missing configuration property '"
                      << pc_config_key.str() << "' for physics constructor '"
                      << pc_name << "' !");
      const std::string& pc_config = config_.fetch_string(pc_config_key.str());
      {
        ...
        DT_THROW_IF(pc_found != _physics_constructors_.end(), std::logic_error,
                    "A physics constructor with name '"
                        << pc_name << "' already exists !");
      ...
      // } else
      {
        ...
        DT_THROW_IF(!_factory_register_.has(pce.id), std::logic_error,
                    "Cannot find physics constructor factory with ID '"
                        ...
                        << pce.name << "' !");
        const base_physics_constructor::factory_register_type::factory_type&
            the_factory = _factory_register_.get(pce.id);
        base_physics_constructor* pc = the_factory();
        DT_THROW_IF(pc == 0, std::logic_error,
                    "Creation of '" << pce.name
                                    << "' physics constructor of type '"
                                    << pce.id << "' failed !");
  ...
  DT_LOG_DEBUG(_logprio(),
               "Use production cuts set to: " << _using_production_cuts_);
  ...
  if (pc_config.size() > 0 && !_using_production_cuts_) {
    DT_LOG_WARNING(_logprio(),
                   "Your configuration has 'production_cuts' related "
                   "parameters but 'using_production_cuts' is not activated!");
  }
  ...
  if (_using_production_cuts_) {
    ...
    if (config_.has_key("production_cuts.length_unit")) {
      std::string lunit_str =
          config_.fetch_string("production_cuts.length_unit");
      lunit = datatools::units::get_length_unit_from(lunit_str);
    }

    // Production cuts energy unit :
    if (config_.has_key("production_cuts.energy_unit")) {
      std::string eunit_str =
          config_.fetch_string("production_cuts.energy_unit");
    ...
    if (config_.has_key("production_cuts.low_energy")) {
      _production_cuts_low_energy_ =
          config_.fetch_real("production_cuts.low_energy");
    ...
    DT_LOG_NOTICE(_logprio(), "Production cuts low edge energy: "
                                  << _production_cuts_low_energy_ / CLHEP::eV
                                  << " eV");

    // Production cuts high edge energy :
    if (config_.has_key("production_cuts.high_energy")) {
      _production_cuts_high_energy_ =
          config_.fetch_real("production_cuts.high_energy");
    ...
    DT_LOG_NOTICE(_logprio(), "Production cuts high edge energy: "
                                  << _production_cuts_high_energy_ / CLHEP::MeV
                                  << " MeV");

    // Default production cut value
    if (config_.has_key("production_cuts.default_value")) {
      _production_cuts_default_value_ =
          config_.fetch_real("production_cuts.default_value");
      DT_THROW_IF(_production_cuts_default_value_ <= 0.0, std::domain_error,
                  "Invalid default cut value ("
                      << _production_cuts_default_value_ << ") !");
    ...
    if (config_.has_key("production_cuts.positron")) {
      _production_cuts_values_.positron =
          config_.fetch_real("production_cuts.positron");
    ...
    if (config_.has_key("production_cuts.proton")) {
      _production_cuts_values_.proton =
          config_.fetch_real("production_cuts.proton");
      if (!config_.has_explicit_unit("production_cuts.proton")) {
        _production_cuts_values_.proton *= lunit;
      }
    }
    DT_LOG_NOTICE(_logprio(), "Proton cut set to: "
                                  << _production_cuts_values_.proton / CLHEP::mm
                                  << " mm");
      ...
      for (int i = 0; i < (int)regions.size(); i++) {
        ...
        if (config_.has_key(key)) {
          ...
          // try gamma:
          {
            ...
            gamma_key_ss << "production_cuts.regions." << region_label
                         << ".gamma";
          ...
          // try electron:
          {
            ...
            electron_key_ss << "production_cuts.regions." << region_label
                            << ".electron";
          ...
          // try positron:
          {
            ...
            positron_key_ss << "production_cuts.regions." << region_label
                            << ".positron";
          ...
          // try proton:
          {
            ...
            proton_key_ss << "production_cuts.regions." << region_label
                          << ".proton";
        ...
        DT_THROW_IF(!checked, std::logic_error,
                    "Missing production cuts value(s)  for region '"
                        << region_label << "' !");
        pc_info.initialize(
            _production_cuts_values_.gamma, _production_cuts_values_.electron,
  ...
  DT_LOG_NOTICE(_logprio(), "Original default cut Length : " << G4BestUnit(
...
void physics_list::_register_physics_constructors() {
  ...
  for (physics_constructor_dict_type::iterator i =
           _physics_constructors_.begin();
       i != _physics_constructors_.end(); i++) {
    physics_constructor_entry& pc_entry = i->second;
    DT_THROW_IF(
        !(pc_entry.status & physics_constructor_entry::STATUS_INITIALIZED),
        std::logic_error,
...
void physics_list::_SetCuts() {
  ...
  DT_LOG_NOTICE(_logprio(), "Default cut Length : " << G4BestUnit(
                                defaultCutValue, "Length"));

  // G4VUserPhysicsList::SetCutsWithDefault method sets the default cut value
  ...
  G4ProductionCutsTable::GetProductionCutsTable()->SetEnergyRange(
      pct_low_edge_energy, pct_high_edge_energy);

  if (_using_production_cuts_) {
    DT_LOG_TRACE(_logprio(), "Using production cuts...");
    // Defining production cuts for different particles:
    // set production cut values for gamma at first and for e- second and next
    // for e+ then proton because some processes for e+/e- need cut values for
    // gamma
    ...
    for (std::map<std::string, production_cuts_info>::const_iterator i =
      ...
      if (a_region != 0) {
        ...
        a_region_cuts->SetProductionCut(pc_info.gamma,
                                        G4ProductionCuts::GetIndex("gamma"));
        a_region_cuts->SetProductionCut(pc_info.electron,
                                        G4ProductionCuts::GetIndex("e-"));
        a_region_cuts->SetProductionCut(pc_info.positron,
                                        G4ProductionCuts::GetIndex("e+"));
        a_region_cuts->SetProductionCut(pc_info.proton,
                                        G4ProductionCuts::GetIndex("proton"));
        a_region->SetProductionCuts(a_region_cuts);
      } else {
        DT_THROW(std::logic_error, "No region named '"
                                       << region_name
                                       << "' to be given production cuts !");
...
void physics_list::tree_dump(std::ostream& out_, const std::string& title_,
  ...
  out_ << indent << datatools::i_tree_dumpable::tag
       << "Initialized                  : " << (_initialized_ ? "Yes" : "No")
       << std::endl;

  // Geant4 physics list:
  out_ << indent << datatools::i_tree_dumpable::tag << "Geant4 physics list  : "
       << (has_geant4_physics_list() ? "Yes" : "No") << std::endl;
  out_ << indent << datatools::i_tree_dumpable::skip_tag
       << datatools::i_tree_dumpable::tag << "Name : '"
       << _geant4_physics_list_name_ << "'" << std::endl;
  ...
  // Production cuts :
  out_ << indent << datatools::i_tree_dumpable::tag
       << "Using production cuts        : "
       << (_using_production_cuts_ ? "Yes" : "No") << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag
       << "Default production cut value : "
...
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(mctools::g4::physics_list, ocd_) {
  ...
  ocd_.set_class_description(
      "The Geant4 simulation manager embedded physics list");
...

  {
    ...
    ld << "Geant4 provides a physics list factory that enables the "
          "instantiation \n"
       << "and initialization of some predefined physics list from its "
          "registration\n"
       << "name.                                                               "
          "  \n"
       << "                                                                    "
    ...
    ld << "                                                                    "
          "  \n";
    ld << "See also: "
          "http://geant4.cern.ch/support/proc_mod_catalog/physics_lists/"
          "useCases.shtml \n";
    ld << "                                                                    "
          "  \n";
    ld << "This property is not compatible with the "
          "``physics_constructors.names``\n"
       << "property.                                                           "
          "   \n";

    cpd.set_name_pattern("geant4.physics_list")
        .set_terse_description(
            "The name of the Geant4 physics list to activate")
...

  {
    ...
    cpd.set_name_pattern("physics_constructors.names")
        .set_terse_description(
            "The names of physics constructor plugins to be loaded")
        .set_traits(datatools::TYPE_STRING,
        ...
        .add_example(
            "                                                                  "
            "          \n"
            " physics_constructors.names : string[2] = \"particles\" "
            "\"electromagnetic\" \n"
            "                                                                  "
            "          \n");
  }

  {
    ...
    cpd.set_name_pattern(
           "physics_constructors.${physics_constructors.names}.id")
  ...
  {
    // Description of the 'physics_constructors.XXX.config' configuration
    // property :
    datatools::configuration_property_description& cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern(
           "physics_constructors.${physics_constructors.names}.config")
        .set_terse_description(
            "The configuration file name of a given physics constructor")
        .set_traits(datatools::TYPE_STRING)
        .set_path(true)
        .set_mandatory(true)
        .add_example(
...

  {
    ...
    cpd.set_name_pattern("production_cuts.regions")
        .set_terse_description(
            "The name of the regions with special production cuts")
        .set_traits(datatools::TYPE_STRING,
                    datatools::configuration_property_description::ARRAY)
        .set_mandatory(false)
        .set_triggered_by_flag("using_production_cuts", true)
  ...
  {
    // Description of the 'production_cuts.regions.XXX.all' configuration
    // property :
    datatools::configuration_property_description& cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern(
           "production_cuts.regions.${production_cuts.regions}.all")
        .set_terse_description(
            "The cut value applied to all particles within a given region")
        .set_traits(datatools::TYPE_REAL)
        .set_explicit_unit(true)
        .set_unit_label("length")
        .set_mandatory(false)
  ...
  {
    // Description of the 'production_cuts.regions.XXX.gamma' configuration
    // property :
    datatools::configuration_property_description& cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern(
           "production_cuts.regions.${production_cuts.regions}.gamma")
        .set_terse_description(
            "The cut value applied to gamma within a given region")
        .set_traits(datatools::TYPE_REAL)
        .set_explicit_unit(true)
        .set_unit_label("length")
        .set_mandatory(false)
  ...
  {
    // Description of the 'production_cuts.regions.XXX.electron' configuration
    // property :
    datatools::configuration_property_description& cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern(
           "production_cuts.regions.${production_cuts.regions}.electron")
        .set_terse_description(
            "The cut value applied to electron within a given region")
        .set_traits(datatools::TYPE_REAL)
        .set_explicit_unit(true)
        .set_unit_label("length")
        .set_mandatory(false)
  ...
  {
    // Description of the 'production_cuts.regions.XXX.positron' configuration
    // property :
    datatools::configuration_property_description& cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern(
           "production_cuts.regions.${production_cuts.regions}.positron")
        .set_terse_description(
            "The cut value applied to positron within a given region")
        .set_traits(datatools::TYPE_REAL)
        .set_explicit_unit(true)
        .set_unit_label("length")
        .set_mandatory(false)
  ...
  ocd_.set_configuration_hints(
      ...
      "  physics_constructors.em.id            : string = "
      "\"mctools::g4::em_physics_constructor\" \n"
...
DOCD_CLASS_SYSTEM_REGISTRATION(mctools::g4::physics_list,
                               "mctools::g4::physics_list")
