...
class run_action : public G4UserRunAction, public loggable_support {
 ...
 private:
  ...
  bool _output_file_preserve_ =
      true;  //!< Flag to preserve an existing data file
  std::string
      _output_file_dir_;  //!< Directory where to store the output data file
  ...
  std::string
      _output_file_compression_;  //!< Compression of the output data file
