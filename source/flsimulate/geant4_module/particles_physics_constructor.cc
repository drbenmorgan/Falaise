...
void particles_physics_constructor::initialize(
  ...
  DT_LOG_DEBUG(_logprio(),
               "Use optical photons set to: " << _use_optical_photons_);
  ...
  DT_LOG_DEBUG(_logprio(),
               "Use strange baryons set to: " << _use_strange_baryons_);
  ...
  DT_LOG_DEBUG(_logprio(),
               "Use bottom baryons set to: " << _use_bottom_baryons_);
  ...
  DT_LOG_DEBUG(_logprio(),
               "He3 is implemented as a generic ion: " << _he3_as_generic_ion_);

  if (config_.has_flag("use_light_anti_nuclei")) {
    _use_light_anti_nuclei_ = true;
  }
  DT_LOG_DEBUG(_logprio(),
               "Use light anti-nuclei set to: " << _use_light_anti_nuclei_);
...
void particles_physics_constructor::tree_dump(std::ostream& out_,
  ...
  out_ << indent << datatools::i_tree_dumpable::tag
       << "Use geantinos                : " << (_use_geantinos_ ? "Yes" : "No")
       << std::endl;

  out_ << indent << datatools::i_tree_dumpable::tag
       << "Use optical photons          : "
       << (_use_optical_photons_ ? "Yes" : "No") << std::endl;

  out_ << indent << datatools::i_tree_dumpable::tag
       << "Use muon leptons             : "
       << (_use_muon_leptons_ ? "Yes" : "No") << std::endl;

  out_ << indent << datatools::i_tree_dumpable::tag
       << "Use tau leptons              : "
       << (_use_tau_leptons_ ? "Yes" : "No") << std::endl;

  out_ << indent << datatools::i_tree_dumpable::tag
       << "Use nucleons                 : " << (_use_nucleons_ ? "Yes" : "No")
       << std::endl;

  out_ << indent << datatools::i_tree_dumpable::tag
       << "Use strange baryons          : "
       << (_use_strange_baryons_ ? "Yes" : "No") << std::endl;

  out_ << indent << datatools::i_tree_dumpable::tag
       << "Use charm baryons            : "
       << (_use_charm_baryons_ ? "Yes" : "No") << std::endl;

  out_ << indent << datatools::i_tree_dumpable::tag
       << "Use bottom baryons           : "
       << (_use_bottom_baryons_ ? "Yes" : "No") << std::endl;

  out_ << indent << datatools::i_tree_dumpable::tag
       << "Use light mesons             : "
       << (_use_light_mesons_ ? "Yes" : "No") << std::endl;

  out_ << indent << datatools::i_tree_dumpable::tag
       << "Use charm mesons             : "
       << (_use_charm_mesons_ ? "Yes" : "No") << std::endl;

  out_ << indent << datatools::i_tree_dumpable::tag
       << "Use bottom mesons            : "
       << (_use_bottom_mesons_ ? "Yes" : "No") << std::endl;

  out_ << indent << datatools::i_tree_dumpable::tag
       << "Use light nuclei             : "
       << (_use_light_nuclei_ ? "Yes" : "No") << std::endl;

  out_ << indent << datatools::i_tree_dumpable::tag
       << "Use light anti-nuclei        : "
       << (_use_light_anti_nuclei_ ? "Yes" : "No") << std::endl;

  out_ << indent << datatools::i_tree_dumpable::inherit_tag(inherit_)
       << "Use generic ion              : "
       << (_use_generic_ion_ ? "Yes" : "No") << std::endl;
...
// From: G4BaryonConstructor::ConstructBottomBaryons();
    ...
    DT_LOG_WARNING(_logprio(), "Geant4 version "
                                   << G4VERSION_NUMBER
                                   << " does not support bottom baryons.");
#endif  // G4VERSION_NUMBER == 960
...
void particles_physics_constructor::_ConstructUserLimits() { return; }
 ...
 */
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(mctools::g4::particles_physics_constructor,
                                ocd_) {
  // The class name :
