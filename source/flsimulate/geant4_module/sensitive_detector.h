...
class sensitive_detector : public G4VSensitiveDetector,
 ...
 public:
  ...
  typedef std::map<std::string, mctools::base_step_hit_processor *>
      hit_processor_dict_type;
 ...
 public:
  ...
  /// Return the list of names of the logical volumes attached to the sensitive
  /// detector
  const std::list<std::string> &get_attached_logical_volumes() const;

  /// Return the mutable list of names of the logical volumes attached to the
  /// sensitive detector
  ...
  /// Return the mutable dictionary of step hit processors attached to the
  /// sensitive detector
  hit_processor_dict_type &grab_hit_processors();

  /// Return the dictionary of step hit processors attached to the sensitive
  /// detector
  ...
  mctools::base_step_hit_processor &grab_hit_processor(
      const std::string &name_);

  /// Build the name of a collection of hits from the name of the sensitive
  /// category
  static std::string make_hit_collection_name(
      const std::string &sensitive_category_);
 ...
 private:
  ...
  std::list<std::string> _attached_logical_volumes_;  //!< The list of geometry
                                                      //!logical volumes
                                                      //!attached to the
                                                      //!sensitive category
  bool _drop_zero_energy_deposit_steps_;              //!< Do not record steps with no
                                                      //!energy deposit
  ...
  bool _record_g4_volume_properties_;  //!< Record Geant4 identifiers of the
                                       //!current volume traversed by the track
                                       //!(string property
                                       //!"sensitive.g4_volume.name" and integer
                                       //!property
                                       //!"sensitive.g4_volume.copy_number")
  bool _record_momentum_;              //!< record the start and stop momentum of the
                                       //!particle
  bool _record_kinetic_energy_;        //!< record the start and stop kinetic energy
                                       //!of the particle
  bool _record_primary_particle_;      //!< Record the boolean property
                                       //!"track.primary"
  bool _record_track_id_;              //!< Record the integer properties "track.id" and
                                       //!"track.parent_id"
  bool _record_major_track_;           //!< Record the boolean property "track.major" for
                              //!tracks of some importance (primary and energy
                              //!above an user threshold)
  double _major_track_minimum_energy_;  //!< Energy threshold to flag "major"
                                        //!tracks
  bool _record_creator_process_;        //!< Record the informations about the creator
                                        //!process (string properties
                                        //!"track.creator_process" and
                                        //!"track.creator_category")
  bool _record_creator_category_;       //!< \deprecated Record the category of the
                                        //!creator process
  bool _record_material_;               //!< Record the current material traversed by the
                           //!track (string property "material.ref")
  bool _record_sensitive_category_;  //!< Record the sensitive category that
                                     //!generated the current hit (string
                                     //!property "sensitive.category")
  bool _record_boundaries_;          //!< Record boolean properties
                                     //!"track.entering_volume" and
                                     //!"track.leaving_volume" respectively when
                                     //!particle enters and leaves the volume
  bool _record_delta_ray_from_alpha_;  //!< Record boolean property
                                       //!"track.delta_ray_from_alpha" for
                                       //!secondary electrons generated as
                                       //!delta-rays along an alpha particle
                                       //!track
  bool _record_step_length_;  //!< Record the real property "track.step_length"
  unsigned int _hits_buffer_capacity_;  //!< The capacity of the pre-allocated
                                        //!buffer of hits

  const track_history::track_info *_track_info_ptr_;  //!< Handle to the
                                                      //!tracking information of
                                                      //!the current track
  const track_history::track_info *_parent_track_info_ptr_;  //!< Handle to the
                                                             //!tracking
                                                             //!information of
                                                             //!the parent track
                                                             //!of the current
  ...
  sensitive_hit_collection
      *_hits_collection_;  //!< Handle to the collection of sensitive hits
