...
class detector_construction : public G4VUserDetectorConstruction,
 ...
 public:
  typedef std::map<std::string, sensitive_detector *>
      sensitive_detector_dict_type;
  ...
  /// Return a non-mutable reference to the collection of embedded sensitive
  /// detectors
 ...
 private:
  ...
  bool _abort_on_error_ = true;  //!< Flag to force abortion on error while
                                 //!building the geometry setup
  ...
  em_field_g4_data_type
      _em_field_g4_data_;  //!< Dictionary of G4 EM field working data
  ...
  datatools::properties
      _biasing_config_;  //!< Configuration properties for the biasing manager
