...
bool primary_generator::is_initialized() const { return _initialized_; }
...
void primary_generator::set_event_action(
    mctools::g4::event_action& event_action_) {
  DT_THROW_IF(_initialized_, std::logic_error,
              "Operation prohibited ! Manager is locked !");
...
void primary_generator::set_vertex_generator(
    ::genvtx::i_vertex_generator& vertex_generator_) {
  DT_THROW_IF(_initialized_, std::logic_error,
              "Operation prohibited ! Manager is locked !");
...
void primary_generator::set_event_generator(
    ::genbb::i_genbb& event_generator_) {
  DT_THROW_IF(_initialized_, std::logic_error,
              "Operation prohibited ! Manager is locked !");
...
size_t primary_generator::get_event_counter() const { return _event_counter_; }
...
void primary_generator::_check() {
  if (has_vertex_generator()) {
    if (!_vertex_generator_->is_initialized()) {
      G4Exception("mctools::g4::primary_generator::_check",
                  "InitializationError", RunMustBeAborted,
  ...
  if (!_event_generator_->is_initialized()) {
    G4Exception("mctools::g4::primary_generator::_check", "InitializationError",
                RunMustBeAborted,
...
void primary_generator::initialize(const datatools::properties& config_) {
  ...
  if (config_.has_key("particle_names_map")) {
    ...
    for (size_t i = 0; i < names.size(); i++) {
      ...
      DT_THROW_IF(genbb_particle_name.empty(), std::logic_error,
                  "Invalid genbb particle name extracted from mapping entry '"
                      << mapping_entry << "' !");
      DT_THROW_IF(g4_particle_name.empty(), std::logic_error,
                  "Invalid Geant4 particle name extracted from mapping entry '"
                      << mapping_entry << "' !");
  ...
  if (config_.has_key("using_bias")) {
    ...
    if (use_bias) {
      ...
      _bias_->set_geometry_manager(
          _run_action_->get_manager().get_geom_manager());
...
void primary_generator::reset() {
  if (!is_initialized()) {
    G4Exception("mctools::g4::primary_generator::reset", "InitializationError",
                FatalException, "Primary generator is not initialized!");
  }
  if (_bias_) {
...
void primary_generator::GeneratePrimaries(G4Event* g4_event_) {
  ...
  // 'event_action::BeginOfEventAction' : Clear event data: FIX BUG: 2011-05-10
  // FM+XG : DO CLEAR THE EVENT DATA MODEL HERE ! clog << "DEVEL:
  // primary_generator::GeneratePrimaries: "
...
void primary_generator::_generate_vertex() {
  ...
  if (_vertex_generator_->is_time_generator()) {
    ...
    _vertex_generator_->shoot_vertex_and_time(mgr.grab_vg_prng(),
                                              _current_vertex_, _current_time_);
  } else {
    // Vertex generator:
...
void primary_generator::_generate_event(G4Event* g4_event_) {
  ...
  if (_bias_) {
    ...
    if (bi.is_killed()) {
      DT_LOG_TRACE(_logprio(),
                   "Event is killed by the primary event biasing algorithm!");
  ...
  for (::genbb::primary_event::particles_col_type::const_iterator
           i = current_generated_event.get_particles().begin();
    ...
    const std::string genbb_particle_label =
    ...
    G4String
        g4_particle_name;  // =
                           // get_g4_particle_name_from_genbb_particle(genbb_particle);
    ...
    if (genbb_particle.has_pdg_code()) {
      ...
      // Support for particle PDG encoding: NOT USABLE YET FOR NOW BECAUSE THIS
      // INTERFACE NEEDS MORE WORKS.
      ...
      // Support for traditional particle 'types' from genbb_help (extended
      // Geant3 codes):
      DT_LOG_DEBUG(_logprio(),
                   "  Attempting to use a GENBB particle with type: "
                       << genbb_particle.get_type());
      if (genbb_particle.is_nucleus()) {
        ...
        */
        ...
        // Fetch the characteristics of the nucleus from the GENBB particle
        // label:
        ...
        if (!nucleus_parsing) {
          ...
          message << "genbb's nucleus particle with label='"
                  ...
                  << "') cannot not be processed as a Geant4 particle !";
          G4Exception("mctools::g4::primary_generator::_generate_event",
                      "InvalidArgument", RunMustBeAborted,
                      message.str().c_str());
        }
        // Instantiate the G4 corresponding ion:
        g4_particle = G4ParticleTable::GetParticleTable()->GetIon(
            Z, A, excitation_energy);
        g4_particle_name = g4_particle->GetParticleName();
        _particle_gun_->SetParticleDefinition(g4_particle);
        DT_LOG_TRACE(
            _logprio(),
        ...
        double excitation_energy = 0.0;
        int ion_charge = 0;
        // Fetch the characteristics of the ion (partially ionized) from the
        // GENBB particle label:
        ...
        DT_LOG_DEBUG(_logprio(), "  Assign dynamic charge ["
                                     << (ion_charge > 0 ? "+" : "")
                                     << ion_charge
                                     << "e] to the Geant4 ion...");
        ...
        if (g4_particle_name[0] == '?') {
          std::ostringstream message;
          message << "genbb's particle type " << genbb_particle.get_type()
                  << " (with label='" << genbb_particle_label
    ...
    if (geomtools::is_valid(_current_vertex_)) {
      // All particles originate from an unique vertex provided by the vertex
      // generator:
      ...
      // Each particle originates from its own vertex provided by the event
      // generator:
    ...
    // other formulas (non relativistic approximation) because of roundoff
    // errors in relativistic formula (is G4 aware of that? to be checked in the
    // G4 code)...
    ...
    if (_logprio() == datatools::logger::PRIO_TRACE) {
      ...
      const std::string skip_tag =
          datatools::i_tree_dumpable::tags::skip_item();
      const std::string last_tag =
          datatools::i_tree_dumpable::tags::last_item();
      const std::string last_skip_tag =
          datatools::i_tree_dumpable::tags::last_skip_item();
      std::string tag1 = tag;
      ...
      std::clog << tag2 << tag
                << "PDG code   = " << genbb_particle.get_pdg_code()
                << std::endl;
      std::clog << tag2 << tag << "Geant4 name  = " << g4_particle_name
                << std::endl;
      std::clog << tag2 << tag << "Time       = "
                << _particle_gun_->GetParticleTime() / CLHEP::ns << " ns"
                << std::endl;
      std::clog << tag2 << tag
                << "Position   = " << _current_vertex_ / CLHEP::mm << " mm"
                << std::endl;
      std::clog << tag2 << tag
                << "Momentum   = " << genbb_particle.get_momentum() / CLHEP::keV
                << " keV" << std::endl;
      if (datatools::is_valid(kinetic_energy)) {
        std::clog << tag2 << last_tag
                  << "Energy     = " << kinetic_energy / CLHEP::keV
                  << " keV (FYI)" << std::endl;
      }
    }
  }  // Particle loop
...
std::string primary_generator::get_g4_particle_name_from_genbb_particle(
  ...
  if (p_.is_ion()) {
    // What to do here ? shoud we support it from this method
    // return "ElA[E*]"; // G4 syntax seems to be : He6[0.0] but there is no way
    // to set the ion charge
