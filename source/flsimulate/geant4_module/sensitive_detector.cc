...
const double sensitive_detector::DEFAULT_MAJOR_TRACK_MINIMUM_ENERGY =
    10. * CLHEP::keV;
...
bool sensitive_detector::has_manager() const { return _manager_ != 0; }
...
const datatools::properties& sensitive_detector::get_auxiliaries() const {
  return _aux_;
}

datatools::properties& sensitive_detector::grab_auxiliaries() { return _aux_; }
...
int sensitive_detector::get_HCID() const { return _HCID_; }
...
mctools::base_step_hit_processor& sensitive_detector::grab_hit_processor(
...
void sensitive_detector::add_hit_processor(
    const std::string& name_, mctools::base_step_hit_processor& shp_) {
  DT_THROW_IF(
      _hit_processors_.find(name_) != _hit_processors_.end(), std::logic_error,
...
void sensitive_detector::attach_logical_volume(
    const std::string& log_volume_name_) {
...
std::list<std::string>& sensitive_detector::grab_attached_logical_volumes() {
  return _attached_logical_volumes_;
}

std::string sensitive_detector::make_hit_collection_name(
    const std::string& name_) {
...
void sensitive_detector::configure(const datatools::properties& config_) {
  ...
  {
    // Record track id
    const std::string trkid_key = geomtools::sensitive::make_key(
        sensitive_utils::SENSITIVE_RECORD_TRACK_ID);
...

  {
    // Record boundaries
    const std::string trkid_key = geomtools::sensitive::make_key(
        sensitive_utils::SENSITIVE_RECORD_BOUNDARIES);
    if (config_.has_flag(trkid_key)) {
...

  {
    ...
    const std::string pp_key = geomtools::sensitive::make_key(
        sensitive_utils::SENSITIVE_RECORD_PRIMARY_PARTICLE);
...

  {
    ...
    const std::string aq_key = geomtools::sensitive::make_key(
        sensitive_utils::SENSITIVE_RECORD_ALPHA_QUENCHING);
  ...
  {
    // Record major track
    const std::string rmt_key = geomtools::sensitive::make_key(
        sensitive_utils::SENSITIVE_RECORD_MAJOR_TRACK);
...

  {
    const std::string mtme_key = geomtools::sensitive::make_key(
        sensitive_utils::SENSITIVE_MAJOR_TRACK_MINIMUM_ENERGY);
...

  {
    ...
    const std::string record_creator_process_key =
        geomtools::sensitive::make_key(
            sensitive_utils::SENSITIVE_RECORD_CREATOR_PROCESS);
...

  {
    ...
    const std::string record_creator_category_key =
        geomtools::sensitive::make_key(
            sensitive_utils::SENSITIVE_RECORD_CREATOR_CATEGORY);
...

  {
    ...
    const std::string record_material_key = geomtools::sensitive::make_key(
        sensitive_utils::SENSITIVE_RECORD_MATERIAL);
...

  {
    ...
    const std::string record_sensitive_category_key =
        geomtools::sensitive::make_key(
            sensitive_utils::SENSITIVE_RECORD_SENSITIVE_CATEGORY);
  ...
  {
    // Record momentum
    const std::string record_momentum_key = geomtools::sensitive::make_key(
        sensitive_utils::SENSITIVE_RECORD_MOMENTUM);
...

  {
    ...
    const std::string record_kinetic_energy_key =
        geomtools::sensitive::make_key(
            sensitive_utils::SENSITIVE_RECORD_KINETIC_ENERGY);
...

  {
    ...
    const std::string record_step_length_key = geomtools::sensitive::make_key(
        sensitive_utils::SENSITIVE_RECORD_STEP_LENGTH);
  ...
  {
    const std::string key = geomtools::sensitive::make_key(
        sensitive_utils::SENSITIVE_HITS_BUFFER_CAPACITY);
  ...
  if (geomtools::sensitive::has_key(config_,
    ...
    const bool flag = geomtools::sensitive::has_flag(
        config_, sensitive_utils::SENSITIVE_TRACK_GAMMA);
    set_track_gamma(flag);
  }

  // Tracking optical photon
  if (geomtools::sensitive::has_key(
          config_, sensitive_utils::SENSITIVE_TRACK_OPTICAL_PHOTON)) {
    const bool flag = geomtools::sensitive::has_flag(
        config_, sensitive_utils::SENSITIVE_TRACK_OPTICAL_PHOTON);
  ...
  if (geomtools::sensitive::has_key(config_,
    ...
    const bool flag = geomtools::sensitive::has_flag(
        config_, sensitive_utils::SENSITIVE_TRACK_NEUTRON);
...
sensitive_detector::sensitive_detector(const std::string& sensitive_category_)
  ...
  // G4 Stuff:
  G4String HCname;
  HCname = sensitive_detector::make_hit_collection_name(_sensitive_category_)
               .c_str();
...
void sensitive_detector::Initialize(
    G4HCofThisEvent* /*some_hit_collections_*/) {
  // cerr << datatools::io::devel << "snemo::g4::sensitive_detector::Initialize:
  // "
  //            << "Entering for detector '" << _sensitive_category_ << "'..."
  //            << endl;
  ...
  if (_record_delta_ray_from_alpha_ || _record_track_id_ ||
      _record_primary_particle_ || _record_major_track_ ||
      _record_creator_process_ || _record_creator_category_) {
...
void sensitive_detector::EndOfEvent(G4HCofThisEvent* some_hit_collections_) {
  ...
  // BUGFIX? FM+AC 2014/09/05: should not be done here but in the
  // event_action(to be validated)
...
G4bool sensitive_detector::ProcessHits(
    G4Step* step_, G4TouchableHistory* /*touchable_history_*/) {
  ...
  if (energy_deposit <= 1.e-10 * CLHEP::keV) {
    ...
    } else if (step_->GetTrack()->GetDefinition() ==
               G4Neutron::NeutronDefinition()) {
      if (_track_neutron_) quit = false;
      //} else if (step_->GetTrack()->GetDefinition() ==
      //G4Neutron::OpticalPhotonDefinition()) {  if (_track_optical_photon_) quit
      // = false;
  ...
  const std::string track_particle_name =
      step_->GetTrack()->GetDefinition()->GetParticleName();
  ...
  */
  ...
  if (_manager_->has_track_history()) {
    if ((_track_info_ptr_ == 0) || (_track_info_ptr_->get_id() != track_id)) {
      ...
      if (!the_track_history.has_track_info(track_id)) {
        ...
        track_history::track_info& ti =
            the_track_history.grab_track_info(track_id);
        ti.set_id(track_id);
    ...
    // MOD: FM+AC 2014-09-05: accept neutral particles as "major" tracks(gamma,
    // neutron...): const bool has_charge
    // =(step_->GetTrack()->GetDynamicParticle()->GetCharge() != 0.0); if
    // (has_charge) {
    if (primary_track) {
      major_track = true;
    }
      ...
      if (track_particle_name == "e-" && !primary_track) {
        ...
        if ((_parent_track_info_ptr_ == 0) ||
          ...
          if (the_track_history.has_track_info(parent_track_id)) {
            ...
            _parent_track_info_ptr_ =
                &the_track_history.grab_track_info(parent_track_id);
          }
        }
  ...
  new_hit->grab_hit_data().set_position_start(
      step_->GetPreStepPoint()->GetPosition());
  new_hit->grab_hit_data().set_position_stop(
      step_->GetPostStepPoint()->GetPosition());
  ...
  if (_record_momentum_) {
    new_hit->grab_hit_data().set_momentum_start(
        step_->GetPreStepPoint()->GetMomentum());
    new_hit->grab_hit_data().set_momentum_stop(
        step_->GetPostStepPoint()->GetMomentum());
  }
  ...
  if (_record_step_length_) {
    hit_aux.store_real(mctools::track_utils::STEP_LENGTH,
                       step_->GetStepLength());
  }
  ...
  if (use_track_info) {
    // special features:
    if (_record_creator_process_ &&
        !_track_info_ptr_->get_creator_process_name().empty()) {
  ...
  if (_record_material_) {
    static std::string material_ref_key = geomtools::material::make_key(
        geomtools::material::material_ref_property());
  ...
  if (_record_sensitive_category_) {
    static std::string sensitive_category_key = geomtools::sensitive::make_key(
        geomtools::sensitive::constants::instance()
            .SENSITIVE_CATEGORY_PROPERTY);
  ...
  if (_record_g4_volume_properties_) {
    ...
    hit_aux.store_string(sensitive_utils::SENSITIVE_G4_VOLUME_NAME_KEY,
                         volume->GetName());
...
void sensitive_detector::tree_dump(std::ostream& out_,
  ...
  // Logical volumes
  {
    out_ << indent << datatools::i_tree_dumpable::tag
         << "Attached logical volumes    : "
         << _attached_logical_volumes_.size() << std::endl;
    for (std::list<std::string>::const_iterator ilog =
             _attached_logical_volumes_.begin();
  ...
  // Flags
  {
    ...
    out_ << indent << datatools::i_tree_dumpable::tag
         << "Track gamma                 : " << (_track_gamma_ ? "Yes" : "No")
         << std::endl;
    ...
    out_ << indent << datatools::i_tree_dumpable::tag
         << "Track neutron               : " << (_track_neutron_ ? "Yes" : "No")
         << std::endl;
    ...
    if (_record_major_track_) {
      out_ << "(Emin > " << _major_track_minimum_energy_ / CLHEP::keV << " keV)"
           << std::endl;
    } else {
      out_ << std::endl;
    }
    ...
    out_ << indent << datatools::i_tree_dumpable::tag
         << "Hits buffer capacity        : " << _hits_buffer_capacity_
         << std::endl;
  }
...
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(mctools::g4::sensitive_detector, ocd_) {
...

  {
    ...
    cpd.set_name_pattern("sensitive.track_gamma")
        .set_terse_description(
            "Record *zero energy deposit* hits from gamma particle")
...

  {
    ...
    cpd.set_name_pattern("sensitive.track_optical_photon")
        .set_terse_description(
            "Record *zero energy deposit* hits from optical photon")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
        .set_long_description(
            "Default value: ``1``                                   \n"
  ...
  {
    // Description of the 'drop_zero_energy_deposit_steps' configuration
    // property :
    datatools::configuration_property_description& cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern("sensitive.drop_zero_energy_deposit_steps")
        .set_terse_description(
            "Do not record *zero energy deposit* hits from particles other "
            "than photons and neutrons")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
        .set_long_description(
            "Default value: ``0``                                   \n"
...

  {
    ...
    cpd.set_name_pattern("sensitive.record_track_id")
        .set_terse_description(
            "Record the 'track.id' and 'track.parent_id' properties")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
        .set_long_description(
            "Example::                                              \n"
...

  {
    ...
    cpd.set_name_pattern("sensitive.record_alpha_quenching")
        .set_terse_description(
            "Record the 'track.delta_ray_from_alpha' flag for secondary "
...

  {
    ...
    cpd.set_name_pattern("sensitive.record_major_track")
        .set_terse_description(
            "Record the 'track.major' flag for primary or high-energy "
            "particles")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
        .set_long_description(
...

  {
    ...
    cpd.set_name_pattern("sensitive.major_track_minimum_energy")
        .set_terse_description(
            "Set the low energy threshold for major charged particles")
...

  {
    ...
    cpd.set_name_pattern("sensitive.record_creator_process")
        .set_terse_description(
            "Record the 'track.creator_process' string property")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
        .set_long_description(
            "Example::                                              \n"
...

  {
    ...
    cpd.set_name_pattern("sensitive.record_kinetic_energy")
        .set_terse_description(
            "Record the 'track.start_kinetic_energy' and "
            "'track.stop_kinetic_energy' real properties")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
        .set_long_description(
            "Example::                                              \n"
...

  {
    ...
    cpd.set_name_pattern("sensitive.record_sensitive_category")
        .set_terse_description(
            "Record the 'sensitive.category' string property")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
        .set_long_description(
            "Example::                                              \n"
...

  {
    ...
    cpd.set_name_pattern("sensitive.record_g4_volume_infos")
        .set_terse_description(
            "Record the 'g4_volume.name' string and 'g4_volume.copy_number' "
            "integer properties")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
        .set_long_description(
...

  {
    ...
    cpd.set_name_pattern("sensitive.hits_buffer_capacity")
        .set_terse_description(
            "Setup the initial capacity of the sensitive hit buffer for memory "
            "optimization")
...

  {
    ...
    cpd.set_name_pattern("sensitive.record_boundaries")
        .set_terse_description(
            "Record the 'track.entering_volume' or 'track.leaving_volume' "
            "flags")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
        .set_long_description(
...
DOCD_CLASS_SYSTEM_REGISTRATION(mctools::g4::sensitive_detector,
                               "mctools::g4::sensitive_detector")
