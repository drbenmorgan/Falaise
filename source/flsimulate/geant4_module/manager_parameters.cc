...
void manager_parameters::tree_dump(std::ostream& out_,
  ...
  out_ << indent << datatools::i_tree_dumpable::tag
       << "g4_visu     = " << g4_visu << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag << "g4_macro    = '"
       << g4_macro << "'" << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag << "logging     = '"
       << logging << "'" << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag
       << "dlls        = " << dlls.size() << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag
       << "manager_config_filename = '" << manager_config_filename << "'"
       << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag
  ...
  out_ << indent << datatools::i_tree_dumpable::tag
       << "mgr_seed                = " << mgr_seed << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag
       << "input_prng_states_file  = '" << input_prng_states_file << "'"
       << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag
       << "init_seed_method        = '" << init_seed_method << "'" << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag
       << "output_prng_states_file = '" << output_prng_states_file << "'"
       << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag
       << "prng_states_save_modulo = " << prng_states_save_modulo << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag
       << "input_prng_seeds_file   = '" << input_prng_seeds_file << "'"
       << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag
       << "output_prng_seeds_file  = '" << output_prng_seeds_file << "'"
       << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag
       << "output_data_format      = '" << output_data_format << "'"
       << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag
       << "output_data_bank_label  = '" << output_data_bank_label << "'"
       << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag
       << "output_data_file        = '" << output_data_file << "'" << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag << "vg_name   = '"
       << vg_name << "'" << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag << "vg_seed   = " << vg_seed
       << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag << "eg_name   = '"
       << eg_name << "'" << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag << "eg_seed   = " << eg_seed
       << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag
       << "shpf_seed = " << shpf_seed << std::endl;
  out_ << indent << datatools::i_tree_dumpable::tag
  ...
  out_ << indent << datatools::i_tree_dumpable::tag
       ...
       << dont_save_no_sensitive_hit_events << std::endl;
...
void manager_parameters::setup(const manager_parameters& a_params,
  ...
  DT_LOG_NOTICE(
      a_manager.get_logging_priority(),
  ...
  if (a_params.mgr_seed != mygsl::random_utils::SEED_INVALID) {
    ...
    a_manager.grab_seed_manager().update_seed(
        mctools::g4::manager::g4_manager_label(), a_params.mgr_seed);
  }

  if (a_params.vg_seed != mygsl::random_utils::SEED_INVALID) {
    // register the vertex generator's seed :
    a_manager.grab_seed_manager().update_seed(
        mctools::g4::manager::vertex_generator_label(), a_params.vg_seed);
  }

  if (a_params.eg_seed != mygsl::random_utils::SEED_INVALID) {
    // register the event generator's seed :
    a_manager.grab_seed_manager().update_seed(
        mctools::g4::manager::event_generator_label(), a_params.eg_seed);
  }

  if (a_params.shpf_seed != mygsl::random_utils::SEED_INVALID) {
    // register the SHPF PRNG's seed :
    a_manager.grab_seed_manager().update_seed(
        mctools::g4::manager::shpf_label(), a_params.shpf_seed);
  }
  ...
  if (!a_params.output_profiles_activation_rule.empty()) {
    ...
    a_manager.set_output_profiles_activation_rule(
        a_params.output_profiles_activation_rule);
  }
