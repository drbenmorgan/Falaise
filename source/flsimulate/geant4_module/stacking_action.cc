...
void stacking_action::initialize(const datatools::properties& config_) {
  ...
  _g4_navigator_ = G4TransportationManager::GetTransportationManager()
                       ->GetNavigatorForTracking();
  return;
}

G4ClassificationOfNewTrack stacking_action::ClassifyNewTrack(
    const G4Track* a_track) {
  ...
  if ((_killer_volume_names_.size() > 0) && (_killer_volumes_.size() == 0)) {
    ...
    for (int i = 0; i < (int)_killer_volume_names_.size(); i++) {
      ...
      G4LogicalVolume* g4_volume_log =
          g4_lv_store->GetVolume(g4_log_name, false);
      DT_THROW_IF(g4_volume_log == 0, std::logic_error,
                  "Logical volume '" << g4_log_name << "' does not exists !");
      DT_LOG_NOTICE(_logprio(),
                    "Add a secondary particle killer logical volume '"
                        << g4_log_name << "'.");
      _killer_volumes_.push_back(g4_volume_log);
    }
  }

  if ((_killer_material_names_.size() > 0) &&
      (_killer_materials_.size() == 0)) {
    // G4LogicalVolumeStore * g4_lv_store = G4LogicalVolumeStore::GetInstance
    // ();
    for (int i = 0; i < (int)_killer_material_names_.size(); i++) {
      ...
      G4Material* g4_material =
          G4Material::GetMaterial(g4_material_name, false);
      DT_THROW_IF(g4_material == 0, std::logic_error,
                  "Material '" << g4_material_name << "' does not exists !");
      DT_LOG_NOTICE(_logprio(), "Add a secondary particle killer material '"
                                    << g4_material_name << "'");
  ...
  // g4_volume_name = _g4_navigator_->LocateGlobalPointAndSetup (pos, NULL,
  // false)->GetName ();

  // Another possibility to get the volume name:
  //   int depth=0;
  //   G4TouchableHistory * theTouchable = (G4TouchableHistory*)
  //   (a_track->GetTouchable ()); if (theTouchable)
  //     {
  ...
  if (_kill_secondary_particles_ && (a_track->GetParentID() > 0)) {
    ...
    // std::cerr << "DEVEL: mctools::g4::stacking::ClassifyNewTrack:
    // g4_volume_log = '"
    ...
    if (!kill) {
      if (g4_volume_log != 0) {
        ...
        if (g4_material != 0) {
          ...
          if (find(_killer_materials_.begin(), _killer_materials_.end(),
    ...
    if (!kill) {
      if (g4_volume_log != 0) {
        ...
        if (find(_killer_volumes_.begin(), _killer_volumes_.end(),
...
void stacking_action::NewStage() { return; }

void stacking_action::PrepareNewEvent() { return; }
...
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(mctools::g4::stacking_action, ocd_) {
...

  {
    ...
    cpd.set_name_pattern("kill_particles.volumes")
        .set_terse_description(
            "The list of logical volumes where secondary particle killing is "
            "activated")
        .set_traits(datatools::TYPE_STRING,
        ...
        .set_long_description(
            ...
            "Example::                       \n"
            "                                \n"
            " kill_particles.volumes : string[2] = \"calorimeter.log\" "
            "\"beam_dump.log\"  \n"
            "                                \n");
  }

  {
    ...
    cpd.set_name_pattern("kill_particles.materials")
        .set_terse_description(
            "The list of materials of which logical volumes are made and where "
  ...
  ocd_.set_configuration_hints(
      ...
      "                                                                       "
      "\n"
      " #@description Activate the killing of secondary particles within some "
      "volumes \n"
      " kill_particles : boolean = 1                                          "
      "\n"
      "                                                                       "
...
DOCD_CLASS_SYSTEM_REGISTRATION(mctools::g4::stacking_action,
                               "mctools::g4::stacking_action")
