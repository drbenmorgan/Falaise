...
DATATOOLS_FACTORY_SYSTEM_AUTO_REGISTRATION_IMPLEMENTATION(
    base_physics_constructor, em_physics_constructor,
    "mctools::g4::em_physics_constructor")

// PIMPL-ized working data:
struct em_physics_constructor::_work_type_ {
  std::map<std::string, ::mctools::g4::processes::em_extra_model>
      em_extra_models;
...
em_physics_constructor::region_deexcitation_type::region_deexcitation_type(
    bool fluo_, bool auger_, bool pixe_) {
  fluorescence = fluo_;
  if (auger_) {
    fluorescence = true;
  }
...
const std::string em_physics_constructor::EM_MODEL_LOW_ENERGY_LIVERMORE =
    "livermore";
const std::string em_physics_constructor::EM_MODEL_LOW_ENERGY_PENELOPE =
    "penelope";

const std::string em_physics_constructor::EM_PIXE_MODEL_EMPIRICAL = "Empirical";
const std::string em_physics_constructor::EM_PIXE_MODEL_ECPSSR_FORMFACTOR =
    "ECPSSR_FormFactor";
const std::string em_physics_constructor::EM_PIXE_MODEL_ECPSSR_ANALYTICAL =
    "ECPSSR_Analytical";

const std::string em_physics_constructor::EM_PIXE_ELECTRON_MODEL_LIVERMORE =
    "Livermore";
const std::string em_physics_constructor::EM_PIXE_ELECTRON_MODEL_PENELOPE =
    "Penelope";
const std::string
    em_physics_constructor::EM_PIXE_ELECTRON_MODEL_PROTON_ANALYTICAL =
        "Analytical";
const std::string
    em_physics_constructor::EM_PIXE_ELECTRON_MODEL_PROTON_EMPIRICAL =
        "Empirical";
...
void em_physics_constructor::initialize(
  ...
  DT_LOG_NOTICE(_logprio(),
                "Default electromagnetic model set to: '" << _em_model_ << "'");

  // Gammas :
  if (config_.has_key("em.gamma.rayleigh_scattering")) {
    _em_gamma_rayleigh_scattering_ =
        config_.fetch_boolean("em.gamma.rayleigh_scattering");
  }
  DT_LOG_DEBUG(_logprio(), "Gamma Rayleigh scattering process set to: "
                               << _em_gamma_rayleigh_scattering_);

  if (config_.has_key("em.gamma.photo_electric")) {
    _em_gamma_photo_electric_ =
        config_.fetch_boolean("em.gamma.photo_electric");
  }
  DT_LOG_DEBUG(_logprio(), "Gamma photoelectric process set to: "
                               << _em_gamma_photo_electric_);

  if (config_.has_key("em.gamma.compton_scattering")) {
    _em_gamma_compton_scattering_ =
        config_.fetch_boolean("em.gamma.compton_scattering");
  }
  DT_LOG_DEBUG(_logprio(), "Gamma Compton scattering process set to: "
                               << _em_gamma_compton_scattering_);
  ...
  DT_LOG_DEBUG(_logprio(),
               "Gamma conversion process set to: " << _em_gamma_conversion_);

  if (config_.has_key("em.gamma.conversion_to_muons")) {
    _em_gamma_conversion_to_muons_ =
        config_.fetch_boolean("em.gamma.conversion_to_muons");
  }
  DT_LOG_DEBUG(_logprio(), "Gamma conversion to muons process set to: "
                               << _em_gamma_conversion_to_muons_);
  ...
  DT_LOG_DEBUG(_logprio(),
               "Gamma step limiter set to: " << _em_gamma_step_limiter_);
  ...
  DT_LOG_DEBUG(_logprio(), "Electron/positron energy loss set to: "
                               << _em_electron_ionization_);
  ...
  if (config_.has_key(
          "em.electron.multiple_scattering.use_distance_to_boundary")) {
    _em_electron_ms_use_distance_to_boundary_ = config_.fetch_boolean(
        "em.electron.multiple_scattering.use_distance_to_boundary");
  }
  DT_LOG_DEBUG(
      _logprio(),
  ...
  if (config_.has_key("em.electron.multiple_scattering.range_factor")) {
    ...
    DT_THROW_IF(
        _em_electron_ms_range_factor_ <= 0.0, std::domain_error,
  ...
  if (config_.has_key("em.electron.bremsstrahlung")) {
    _em_electron_bremsstrahlung_ =
        config_.fetch_boolean("em.electron.bremsstrahlung");
  }
  DT_LOG_DEBUG(_logprio(), "Electron/positron bremsstrahlung set to: "
                               << _em_electron_bremsstrahlung_);

  // Positrons :
  if (config_.has_key("em.positron.annihilation")) {
    _em_positron_annihilation_ =
        config_.fetch_boolean("em.positron.annihilation");
  }
  DT_LOG_DEBUG(_logprio(),
               "Positron annihilation set to: " << _em_positron_annihilation_);

  if (config_.has_key("em.electron.step_limiter")) {
    _em_electron_step_limiter_ =
        config_.fetch_boolean("em.electron.step_limiter");
  }
  DT_LOG_DEBUG(_logprio(), "Electron/positron step limiter set to: "
                               << _em_electron_step_limiter_);
  ...
  if (config_.has_key("em.deexcitation.pixe.cross_section_model")) {
    ...
    DT_THROW_IF(
        _em_pixe_cross_section_model_ != EM_PIXE_MODEL_EMPIRICAL &&
        ...
        "Invalid EM PIXE cross-section model '" << _em_pixe_cross_section_model_
                                                << "' !");
    DT_LOG_DEBUG(_logprio(), "EM PIXE cross-section model set to : "
                                 << _em_pixe_cross_section_model_);
  }

  if (config_.has_key("em.deexcitation.pixe.electron_cross_section_model")) {
    _em_pixe_electron_cross_section_model_ = config_.fetch_string(
        "em.deexcitation.pixe.electron_cross_section_model");
    DT_THROW_IF(_em_pixe_electron_cross_section_model_ !=
                        EM_PIXE_ELECTRON_MODEL_LIVERMORE &&
                    _em_pixe_electron_cross_section_model_ !=
                        EM_PIXE_ELECTRON_MODEL_PROTON_ANALYTICAL &&
                    _em_pixe_electron_cross_section_model_ !=
                        EM_PIXE_ELECTRON_MODEL_PROTON_EMPIRICAL &&
                    _em_pixe_electron_cross_section_model_ !=
                        EM_PIXE_ELECTRON_MODEL_PENELOPE,
                std::logic_error,
                "Invalid EM PIXE e+/e- cross-section model '"
                    << _em_pixe_electron_cross_section_model_ << "' !");
  ...
  for (size_t i = 0; i < deexcitation_regions.size(); i++) {
    ...
    region_fluo_key << "em.deexcitation.regions." << region_name
                    << ".fluorescence";
  ...
  if (config_.has_key("em.ion.multiple_scattering")) {
    _em_ion_multiple_scattering_ =
        config_.fetch_boolean("em.ion.multiple_scattering");
  }
  DT_LOG_DEBUG(_logprio(), "Ion multiple scattering set to: "
                               << _em_ion_multiple_scattering_);
  ...
  DT_LOG_DEBUG(_logprio(),
               "Muon bremsstrahlung set to: " << _em_muon_bremsstrahlung_);

  if (config_.has_key("em.muon.multiple_scattering")) {
    _em_muon_multiple_scattering_ =
        config_.fetch_boolean("em.muon.multiple_scattering");
  }
  DT_LOG_DEBUG(_logprio(), "Muon multiple scattering set to: "
                               << _em_muon_multiple_scattering_);

  if (config_.has_key("em.muon.pair_production")) {
    _em_muon_pair_production_ =
        config_.fetch_boolean("em.muon.pair_production");
  }
  DT_LOG_DEBUG(_logprio(),
               "Muon pair production set to: " << _em_muon_pair_production_);

  if (config_.has_key("em.muon.step_limiter")) {
    _em_muon_step_limiter_ = config_.fetch_boolean("em.muon.step_limiter");
  }
  DT_LOG_DEBUG(_logprio(),
               "Muon step limiter set to: " << _em_muon_step_limiter_);

  if (config_.has_key("em.muon.user_special_cuts")) {
    _em_muon_user_special_cuts_ =
        config_.fetch_boolean("em.muon.user_special_cuts");
  }
  DT_LOG_DEBUG(_logprio(), "Muon user special cuts set to: "
                               << _em_muon_user_special_cuts_);
  ...
  DT_LOG_DEBUG(_logprio(),
               "Other ionization set to: " << _em_other_ionization_);

  if (config_.has_key("em.other.multiple_scattering")) {
    _em_other_multiple_scattering_ =
        config_.fetch_boolean("em.other.multiple_scattering");
  }
  DT_LOG_DEBUG(_logprio(), "Other multiple scattering set to: "
                               << _em_other_multiple_scattering_);
  ...
  DT_LOG_DEBUG(_logprio(),
               "Other step limiter set to: " << _em_other_step_limiter_);

  if (config_.has_key("em.other.user_special_cuts")) {
    _em_other_user_special_cuts_ =
        config_.fetch_boolean("em.other.user_special_cuts");
  }
  DT_LOG_DEBUG(_logprio(), "Other user special cuts set to: "
                               << _em_other_user_special_cuts_);
  ...
  DT_LOG_DEBUG(_logprio(),
               "Using extra models set to: " << _em_using_extra_models_);
...
void em_physics_constructor::_setup_em_extra_models_configurator(
  ...
  if (config_.has_key("em.configurator.verbosity")) {
    std::string verbosity_label =
        config_.fetch_string("em.configurator.verbosity");
    if (verbosity_label == "mute") {
      ...
      DT_THROW(std::logic_error, "Invalid EM configurator verbosity '"
                                     << verbosity_label << "'!");
    }
  }
  ...
  config_.export_and_rename_starting_with(extra_models_config,
                                          "em.extra_models.", "");
  ...
  if (extra_models_config.has_key("labels")) {
    ...
    for (int i = 0; i < (int)em_xmodel_labels.size(); i++) {
      ...
      DT_LOG_NOTICE(_logprio(), "Fetch parameters for EM extra model '"
                                    << em_xmodel_label << "'...");
      em_xmodel.initialize(extra_model_config);
      DT_LOG_DEBUG(_logprio(), "EM extra model '" << em_xmodel_label
                                                  << "' parameters are : ");
  ...
  DT_LOG_DEBUG(_logprio(), "Apply EM extra models to the Geant4 EM
  configurator..."); for (std::map<std::string,
  ::mctools::g4::processes::em_extra_model>::const_iterator i =
  _grab_work_().em_extra_models.begin(); i !=
  _grab_work_().em_extra_models.end(); i++){ const std::string & em_xmodel_label
  = i->first; const ::mctools::g4::processes::em_extra_model & em_xmodel =
  i->second;
  ...
  DT_LOG_DEBUG(_logprio(), "EM extra models have been applied to the Geant4 EM
  configurator.");
  */
...
void em_physics_constructor::_set_defaults() {
  ...
  _em_pixe_electron_cross_section_model_ =
      EM_PIXE_ELECTRON_MODEL_LIVERMORE;  // clear();
...
void em_physics_constructor::tree_dump(std::ostream& out_,
  ...
  out_ << indent << datatools::i_tree_dumpable::tag
       << "Gamma step limiter : " << (_em_gamma_step_limiter_ ? "Yes" : "No")
       << std::endl;
  ...
  out_ << indent << datatools::i_tree_dumpable::tag
       ...
       << (_em_electron_ms_use_distance_to_boundary_ ? "Yes" : "No")
       << std::endl;
  ...
  out_ << indent << datatools::i_tree_dumpable::skip_tag
       << datatools::i_tree_dumpable::tag << "PIXE cross section model : "
       << "'" << _em_pixe_cross_section_model_ << "'" << std::endl;

  out_ << indent << datatools::i_tree_dumpable::skip_tag
       << datatools::i_tree_dumpable::tag << "PIXE e+/e- cross section model : "
  ...
  // Ion:
  out_ << indent << datatools::i_tree_dumpable::tag
       << "Ion ionization : " << (_em_ion_ionization_ ? "Yes" : "No")
       << std::endl;
  ...
  out_ << indent << datatools::i_tree_dumpable::tag
       << "Ion step limiter : " << (_em_ion_step_limiter_ ? "Yes" : "No")
       << std::endl;
  ...
  // Muon:
  out_ << indent << datatools::i_tree_dumpable::tag
       << "Muon ionization : " << (_em_muon_ionization_ ? "Yes" : "No")
       << std::endl;

  out_ << indent << datatools::i_tree_dumpable::tag
       << "Muon bremsstrahlung : " << (_em_muon_bremsstrahlung_ ? "Yes" : "No")
       << std::endl;

  out_ << indent << datatools::i_tree_dumpable::tag << "Muon pair production : "
  ...
  out_ << indent << datatools::i_tree_dumpable::tag
       << "Muon step limiter : " << (_em_muon_step_limiter_ ? "Yes" : "No")
       << std::endl;
  ...
  out_ << indent << datatools::i_tree_dumpable::tag
       << "Other ionization : " << (_em_other_ionization_ ? "Yes" : "No")
       << std::endl;
  ...
  out_ << indent << datatools::i_tree_dumpable::tag
       << "Other step limiter : " << (_em_other_step_limiter_ ? "Yes" : "No")
       << std::endl;
  ...
  out_ << indent << datatools::i_tree_dumpable::inherit_tag(inherit_)
       << "Using EM extra models : " << (_em_using_extra_models_ ? "Yes" : "No")
       << std::endl;
...
void em_physics_constructor::ConstructProcess() {
  ...
  if (_em_fluorescence_ || _em_auger_ || _em_pixe_ ||
      _em_regions_deexcitation_.size() > 0) {
...
void em_physics_constructor::_ConstructExtraModels() {
  ...
  DT_LOG_NOTICE(_logprio(), "Setup the EM configurator...");
  _grab_work_().em_configurator.SetVerbose(
      _grab_work_().em_configurator_verbosity);
  DT_LOG_DEBUG(_logprio(), "Initialize the EM model factory...");
  _grab_work_().em_model_fact.initialize();

  DT_LOG_DEBUG(_logprio(),
               "Apply EM extra models to the Geant4 EM configurator...");
  for (std::map<std::string,
    ...
    // Declare extra models to the embedded Geant4 EM configurator:
    DT_LOG_NOTICE(_logprio(), "Declare extra model '"
                                  << em_xmodel_label
                                  << "' in the Geant4 EM configurator...");
    em_xmodel.apply_to_g4(_grab_work_().em_configurator,
                          _grab_work_().em_model_fact);
  ...
  DT_LOG_DEBUG(
      _logprio(),
      "EM extra models have been applied to the Geant4 EM configurator.");
  ...
  */
  ...
  for (std::map<std::string, region_deexcitation_type>::const_iterator i =
    ...
    DT_THROW_IF(a_region == 0, std::logic_error,
                "Cannot find region named '"
                    << region_name << "' to apply de-excitation processes !");
    DT_LOG_NOTICE(_logprio(), "Activating EM deexcitation for region '"
                                  << region_name << "' :"
                                  << " Fluorescence=" << rd.fluorescence
                                  << " Auger=" << rd.auger
                                  << " PIXE=" << rd.pixe << "...");
    emOptions.SetDeexcitationActiveRegion(region_name, rd.fluorescence,
                                          rd.auger, rd.pixe);
  }
    ...
    if (particle_name == "gamma") {
      ...
      if (is_em_low_energy_livermore()) {
        ...
        if (the_rayleigh_scattering) {
          ...
          DT_LOG_NOTICE(_logprio(),
                        "Setup Livermore Rayleigh scattering model for '"
                            << particle_name << "'");
        }
        ...
        if (the_photoelectric_effect) {
          ...
          the_photoelectric_effect->SetEmModel(
              the_livermore_photoelectric_model);
          DT_LOG_NOTICE(_logprio(),
                        "Setup Livermore photoelectric effect model for '"
                            << particle_name << "'");
        }
        ...
        if (the_compton_scattering) {
          ...
          DT_LOG_NOTICE(_logprio(),
                        "Setup Livermore Compton scattering model for '"
                            << particle_name << "'");
        }

        if (the_gamma_conversion) {
          G4LivermoreGammaConversionModel*
              the_livermore_gamma_conversion_model =
                  new G4LivermoreGammaConversionModel();
          the_gamma_conversion->SetEmModel(
              the_livermore_gamma_conversion_model);
          DT_LOG_NOTICE(_logprio(), "Setup Livermore conversion model for '"
                                        << particle_name << "'");
        }
        ...
        if (the_rayleigh_scattering) {
          ...
          DT_LOG_NOTICE(_logprio(),
                        "Setup Penelope Rayleigh scattering model for '"
                            << particle_name << "'");
        }
        ...
        if (the_photoelectric_effect) {
          ...
          the_photoelectric_effect->SetEmModel(
              the_penelope_photoelectric_model);
          DT_LOG_NOTICE(_logprio(),
                        "Setup Penelope photoelectric effect model for '"
                            << particle_name << "'");
        }
        ...
        if (the_compton_scattering) {
          ...
          DT_LOG_NOTICE(_logprio(),
                        "Setup Penelope Compton scattering model for '"
                            << particle_name << "'");
        }
        ...
        if (the_gamma_conversion) {
          ...
          DT_LOG_NOTICE(_logprio(), "Setup Penelope conversion model for '"
                                        << particle_name << "'");
        }
      }
      ...
      if (_em_gamma_step_limiter_) {
        ++process_rank;
        DT_LOG_NOTICE(_logprio(),
                      "Add step limiter for '" << particle_name << "'");
        pmanager->AddProcess(new G4StepLimiter, -1, -1, process_rank);
      }

      if (_em_gamma_user_special_cuts_) {
        ++process_rank;
        DT_LOG_NOTICE(_logprio(), "Add user special cuts limiter for '"
                                      << particle_name << "'");
      ...
      if (_em_electron_multiple_scattering_) {
        // Multiple scattering:
        G4eMultipleScattering* the_electron_multiple_scattering =
            new G4eMultipleScattering();
        // Setting the FacRange to 0.005 instead of default value 0.2
        the_electron_multiple_scattering->SetRangeFactor(
            _em_electron_ms_range_factor_);
        //???
        //the_electron_multiple_scattering->SetStepLimitType(fUseDistanceToBoundary);
        ++process_rank;
        DT_LOG_NOTICE(_logprio(), "Add e+/e- multiple scattering for '"
                                      << particle_name << "'");
        pmanager->AddProcess(the_electron_multiple_scattering, -1, process_rank,
                             process_rank);
      }
      ...
      if (_em_electron_ionization_) {
        ...
        ++process_rank;
        DT_LOG_NOTICE(_logprio(),
                      "Add e+/e- ionization for '" << particle_name << "'");
      ...
      if (_em_electron_bremsstrahlung_) {
        ...
        if (is_em_low_energy_livermore()) {
          ...
          if (particle_name == "e-") {
            ...
            the_e_bremsstrahlung->SetEmModel(
                the_livermore_bremsstrahlung_model);
          } else if (particle_name == "e+") {
        ...
        ++process_rank;
        DT_LOG_NOTICE(_logprio(),
                      "Add e+/e- bremsstrahlung for '" << particle_name << "'");
        pmanager->AddProcess(the_e_bremsstrahlung, -1, process_rank,
                             process_rank);
      }

      if (particle_name == "e+" && _em_positron_annihilation_) {
        // e+ annihilation:
        G4eplusAnnihilation* the_positron_annihilation =
        ...
        if (is_em_low_energy_penelope()) {
          ...
          the_positron_annihilation->SetEmModel(
              the_penelope_annihilation_model);
        }
        ++process_rank;
        DT_LOG_NOTICE(_logprio(), "Add positron annihilation for '"
                                      << particle_name << "'");
      ...
      if (_em_electron_user_special_cuts_) {
        ++process_rank;
        DT_LOG_NOTICE(_logprio(),
                      "Add user special cuts for '" << particle_name << "'");
        pmanager->AddProcess(new G4UserSpecialCuts, -1, -1, process_rank);
      }

    } else if (particle_name == "alpha" || particle_name == "anti_alpha" ||
               particle_name == "deuteron" ||
               particle_name == "anti_deuteron" || particle_name == "triton" ||
               particle_name == "anti_triton" || particle_name == "He3" ||
               particle_name == "anti_He3" || particle_name == "GenericIon") {
      int process_rank = 0;
      /************
       *   Ions   *
       ************/
      if (_em_ion_multiple_scattering_) {
        G4hMultipleScattering* the_ion_multiple_scattering =
            new G4hMultipleScattering();
        ++process_rank;
        DT_LOG_NOTICE(_logprio(), "Add hadron multiple scattering for '"
                                      << particle_name << "'");
        pmanager->AddProcess(the_ion_multiple_scattering, -1, process_rank,
                             process_rank);
      }
      ...
      if (_em_ion_ionization_) {
        ...
        // the energy loss process uses an effectuve charge to compute the
        // energy loss in any materials, particularly in solid state. This has
        // the effect to change the effective charge of the ion with some
        // arbitrary value (example: +6e -> +1e). This is a priori not a problem
        // in solid or liquid materials but the problem comes with low density
        // materials (vacuum, gas) where the charge is changed regardless of the
        // effective density and step length along the particle track. Now if an
        // electric field is active in the some gas volume, the electric force
        // computation is done with the changed (and wrong) effective charge and
        // thus this results in an not correct deflection of the ion track.
        // There is a solution to this issue: we can ask the particle to be
        // associated with a suitable ionization model for this specific
        // category of material. The G4BraggIonGasModel and
        // G4BetheBlochIonGasModel do not use any effective charge of the ion
        // and thus the electric force should be correctly computed in such
        // medium. This implies to change the default ionization models for
        // ions.
        //
        ...
        if (effective_charge_in_gas) {
          ...
          DT_LOG_DEBUG(_logprio(),
                       "Using special ion ionization model in gas...");
          G4BraggIonGasModel* bragg_ion_model = new G4BraggIonGasModel;
          the_ion_ionization->SetEmModel(bragg_ion_model, 1);
          G4BetheBlochIonGasModel* bethe_bloch_model =
        ...
        ++process_rank;
        DT_LOG_NOTICE(_logprio(),
                      "Add ion ionization for '" << particle_name << "'");
        pmanager->AddProcess(the_ion_ionization, -1, process_rank,
                             process_rank);
      }
      ...
      if (_em_muon_multiple_scattering_) {
        ++process_rank;
        DT_LOG_NOTICE(_logprio(), "Add muon multiple scattering for '"
                                      << particle_name << "'");
        pmanager->AddProcess(new G4MuMultipleScattering, -1, process_rank,
                             process_rank);
      }
      ...
      if (_em_muon_ionization_) {
        ++process_rank;
        DT_LOG_NOTICE(_logprio(),
                      "Add muon ionization for '" << particle_name << "'");
        pmanager->AddProcess(new G4MuIonisation, -1, process_rank,
                             process_rank);
      }
      ...
      if (_em_muon_bremsstrahlung_) {
        ++process_rank;
        DT_LOG_NOTICE(_logprio(),
                      "Add muon bremsstrahlung for '" << particle_name << "'");
        pmanager->AddProcess(new G4MuBremsstrahlung, -1, process_rank,
                             process_rank);
      }
      ...
      if (_em_muon_pair_production_) {
        ++process_rank;
        DT_LOG_NOTICE(_logprio(),
                      "Add muon pair production '" << particle_name << "'");
        pmanager->AddProcess(new G4MuPairProduction, -1, process_rank,
                             process_rank);
      }
      ...
      if ((!particle->IsShortLived()) && (particle->GetPDGCharge() != 0.0)) {
        if (_em_other_multiple_scattering_) {
          ...
          DT_LOG_NOTICE(_logprio(), "Add hadron multiple scattering for '"
                                        << particle_name << "'");
          pmanager->AddProcess(new G4hMultipleScattering, -1, process_rank,
                               process_rank);
        }
        ...
        if (_em_other_ionization_) {
          ++process_rank;
          DT_LOG_NOTICE(_logprio(),
                        "Add hadron ionization for '" << particle_name << "'");
          pmanager->AddProcess(new G4hIonisation, -1, process_rank,
                               process_rank);
        }
...
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(mctools::g4::em_physics_constructor, ocd_) {
...

  {
    ...
    cpd.set_name_pattern("em.gamma.photo_electric")
        .set_terse_description(
            "Flag to activate the gamma photo-electric effect")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
...

  {
    ...
    cpd.set_name_pattern("em.electron.ionization")
        .set_terse_description(
            "Flag to activate the electron/positron ionization")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
        .set_default_value_boolean(true)
        .add_example(
...

  {
    ...
    cpd.set_name_pattern("em.electron.bremsstrahlung")
        .set_terse_description(
            "Flag to activate the bremsstrahlung effect for electron/positron")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
        .set_default_value_boolean(true)
        .add_example(
  ...
  {
    // Description of the 'em.electron.multiple_scattering' configuration
    // property :
    datatools::configuration_property_description& cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern("em.electron.multiple_scattering")
        .set_terse_description(
            "Flag to activate the electron/positron multiple scattering")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
        .set_default_value_boolean(true)
        .add_example(
  ...
  {
    // Description of the
    // 'em.electron.multiple_scattering.use_distance_to_boundary' configuration
    // property :
    datatools::configuration_property_description& cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern(
           "em.electron.multiple_scattering.use_distance_to_boundary")
        .set_terse_description(
            "Flag to use distance to boundary in the electron/positron "
            "multiple scattering")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
        .set_default_value_boolean(true)
        .add_example(
            "Use the distance to boundary algorithm for electron/positron "
            "multiple scattering : :: \n"
            "                                                                  "
            "       \n"
            " em.electron.multiple_scattering.use_distance_to_boundary : "
  ...
  {
    // Description of the 'em.electron.multiple_scattering.range_factor'
    // configuration property :
    datatools::configuration_property_description& cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern("em.electron.multiple_scattering.range_factor")
        .set_terse_description(
            "The range factor in the electron/positron multiple scattering")
...

  {
    ...
    cpd.set_name_pattern("em.electron.step_limiter")
        .set_terse_description(
            "Flag to activate the electron/positron step limiter")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
        .set_default_value_boolean(true)
        .add_example(
...

  {
    ...
    cpd.set_name_pattern("em.deexcitation.fluorescence")
        .set_terse_description(
            "Flag to activate the atomic deexcitation and fluorescence")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
        .set_default_value_boolean(false)
        .add_example(
...

  {
    ...
    cpd.set_name_pattern("em.deexcitation.auger")
        ...
        .set_default_value_boolean(false)
        .set_long_description(
            "Auger effect is only activated if "
            "``em.deexcitation.fluorescence``\n"
            "is also activated.                                                "
            "\n")
        .add_example(
...

  {
    ...
    cpd.set_name_pattern("em.deexcitation.pixe")
        .set_terse_description(
            "Flag to activate PIXE process (Particle Induced X-ray Emission)")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
        .set_default_value_boolean(false)
        .add_example(
  ...
  {
    // Description of the 'em.deexcitation.pixe.cross_section_model'
    // configuration property :
    datatools::configuration_property_description& cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern("em.deexcitation.pixe.cross_section_model")
        ...
        .add_example(
            ...
            "                                                       \n"
            " em.deexcitation.pixe.cross_section_model : string = "
            "\"empirical\"  \n"
            "                                                       \n");
  }

  {
    ...
    cpd.set_name_pattern("em.deexcitation.pixe.electron_cross_section_model")
        ...
        .add_example(
            "Use the *empirical* PIXE model : :: \n"
            "                                                       \n"
            " em.deexcitation.pixe.electron_cross_section_model : string = "
            "\"livermore\"  \n"
            "                                                       \n");
  }

  {
    ...
    cpd.set_name_pattern("em.deexcitation.regions")
        .set_terse_description(
            "The name of the regions with special parameters for deexcitation "
            "processes")
        .set_traits(datatools::TYPE_STRING,
  ...
  {
    // Description of the 'em.deexcitation.regions.XXX.fluorescence'
    // configuration property :
    datatools::configuration_property_description& cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern(
           "em.deexcitation.regions.${em.deexcitation.regions}.fluorescence")
        .set_terse_description(
            "Flag to activate fluorecence in some given region")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
        .set_default_value_boolean(false)
        .add_example(
  ...
  {
    // Description of the 'em.deexcitation.regions.XXX.auger' configuration
    // property :
    datatools::configuration_property_description& cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern(
           "em.deexcitation.regions.${em.deexcitation.regions}.auger")
        .set_terse_description(
            "Flag to activate Auger effect in some given region")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
        .set_default_value_boolean(false)
        .add_example(
  ...
  {
    // Description of the 'em.deexcitation.regions.XXX.pixe' configuration
    // property :
    datatools::configuration_property_description& cpd =
        ocd_.add_property_info();
    cpd.set_name_pattern(
           "em.deexcitation.regions.${em.deexcitation.regions}.pixe")
        .set_terse_description(
            "Flag to activate PIXE effect in some given region")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
        .set_default_value_boolean(false)
        .add_example(
