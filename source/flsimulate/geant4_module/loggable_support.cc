...
void loggable_support::set_logging_priority(
    const std::string& priority_label_) {
  datatools::logger::priority p =
      datatools::logger::get_priority(priority_label_);
...
void loggable_support::_initialize_logging_support(
    const datatools::properties& config_) {
  set_logging_priority(datatools::logger::extract_logging_configuration(
      config_, datatools::logger::PRIO_FATAL));
  return;
}
