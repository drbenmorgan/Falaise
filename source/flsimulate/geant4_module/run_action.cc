...
bool run_action::is_initialized() const { return _initialized_; }
...
manager& run_action::grab_manager() { return *_manager_; }

const manager& run_action::get_manager() const { return *_manager_; }

bool run_action::save_data() const { return _save_data_; }
...
const dpp::output_module& run_action::get_out_module() const {
  DT_THROW_IF(!has_out_module(), std::logic_error,
              "Run action has no embedded output module!");
...
const brio::writer& run_action::get_brio_writer() const {
  DT_THROW_IF(!has_brio_writer(), std::logic_error,
              "Run action has no embedded brio writer!");
...
datatools::data_writer& run_action::grab_writer() {
  DT_THROW_IF(!has_writer(), std::logic_error,
              "Run action has no embedded writer!");
...
void run_action::_set_default() {
  ...
  _brio_plain_simulated_data_store_label_ =
      io_utils::PLAIN_SIMULATED_DATA_STORE;
  return;
}
...
void run_action::initialize(const datatools::properties& a_config) {
  ...
  if (!has_number_of_events_modulo()) {
    if (a_config.has_key("number_of_events_modulo")) {
      const int number_of_events_modulo =
          a_config.fetch_integer("number_of_events_modulo");
  ...
  if (_save_data_) {
    if (_output_data_format_ == io_utils::DATA_FORMAT_INVALID) {
      if (a_config.has_key("file.data_format")) {
        const std::string& data_format_label =
            a_config.fetch_string("file.data_format");
  ...
  if (grab_manager().has_simulation_ctrl()) {
    DT_LOG_NOTICE(_logprio(),
                  "Inhibit plain data saving in thread-driven mode !");
    _save_data_ = false;
  }
...
void run_action::dump(std::ostream& a_out) const {
  ...
  a_out << "|-- Output data format  : '" << _output_data_format_ << "'"
        << std::endl;
  a_out << "|-- Output data bank label : '" << _output_data_bank_label_ << "'"
        << std::endl;
  a_out << "|-- Output file dir     : '" << _output_file_dir_ << "'"
        << std::endl;
  a_out << "|-- Output file prefix  : '" << _output_file_prefix_ << "'"
        << std::endl;
  a_out << "|-- Output file format  : '" << _output_file_format_ << "'"
        << std::endl;
  a_out << "|-- Output compression  : '" << _output_file_compression_ << "'"
        << std::endl;
...
void run_action::_build_run_footer() {
  ...
  _run_footer_.store("number_of_processed_events",
                     _number_of_processed_events_);
  _run_footer_.store("number_of_saved_events", _number_of_saved_events_);

  DT_LOG_NOTICE(_logprio(), "Number of processed events = "
                                << _number_of_processed_events_);
  if (_save_data_) {
    DT_LOG_NOTICE(_logprio(),
                  "Number of saved events = " << _number_of_saved_events_);
  }
  return;
}
...
void run_action::BeginOfRunAction(const G4Run* a_run) {
  ...
  if (get_manager().has_simulation_ctrl()) {
    ...
    simulation_ctrl& SimCtrl = grab_manager().grab_simulation_ctrl();
    {
      ...
      while (SimCtrl.event_availability_status ==
             simulation_ctrl::NOT_AVAILABLE_FOR_G4) {
  ...
  if (_save_data_) {
    ...
    if (_output_file_.empty()) {
      DT_LOG_DEBUG(_logprio(),
                   "Trying to build the filename of the output data file...");
      ...
      if (_output_file_format_ == "ascii") {
        output_file_oss << '.'
                        << datatools::io_factory::format::text_extension();
      } else if (_output_file_format_ == "binary") {
        output_file_oss << '.'
                        << datatools::io_factory::format::binary_extension();
      } else if (_output_file_format_ == "xml") {
        output_file_oss << '.'
                        << datatools::io_factory::format::xml_extension();
      ...
      if (!using_brio) {
        if (_output_file_compression_ == "gzip") {
          output_file_oss << '.'
                          << datatools::io_factory::format::gzip_extension();
        } else if (_output_file_compression_ == "bzip2") {
          output_file_oss << '.'
                          << datatools::io_factory::format::bzip2_extension();
        }
      }
      _output_file_ = output_file_oss.str();
      DT_LOG_DEBUG(_logprio(), "The filename of the output data is built: '"
                                   << _output_file_ << "'");
    ...
    if (_output_data_format_ == io_utils::DATA_FORMAT_BANK) {
      ...
      _io_work_->out_module->set_preserve_existing_output(
          _output_file_preserve_);
      // Metadata XXX
      ...
      if (!output_file_directives) {
        ...
        const std::string extension =
            boost::filesystem::extension(output_file_name);
        if (extension == brio::store_info::constants::brio_file_extension()) {
          using_brio = true;
        } else if (extension ==
                   brio::store_info::constants::trio_file_extension()) {
          using_brio = true;
        }
      }
      if (using_brio) {
        DT_LOG_DEBUG(_logprio(),
                     "'Plain' data format will use Brio based archives.");
      }
      if (using_run_header_footer()) {
        // Rebuild the run header:
        _run_header_.clear();
      ...
      if (using_brio) {
        ...
        DT_LOG_NOTICE(_logprio(),
                      "Opening brio serialization output data file '"
                          << output_file_name << "'...");
        DT_LOG_DEBUG(_logprio(), "Brio writer is opened: '"
                                     << _io_work_->brio_writer->is_opened()
                                     << "'...");
        if (_io_work_->brio_writer->is_locked())
          _io_work_->brio_writer->unlock();
        _io_work_->brio_writer->set_existing_file_protected(
            _output_file_preserve_);
        _io_work_->brio_writer->open(output_file_name);
        DT_LOG_NOTICE(_logprio(),
                      "Output data file '" << output_file_name << "' is open.");
        if (using_run_header_footer()) {
          DT_LOG_DEBUG(_logprio(), "Brio writer has store '"
                                       << _brio_general_info_store_label_
                                       << "' : "
                                       << _io_work_->brio_writer->has_store(
                                              _brio_general_info_store_label_));
        ...
        _io_work_->brio_writer->add_store(store,
        ...
        DT_LOG_NOTICE(_logprio(),
                      "Opening datatools serialization output data file '"
                          << output_file_name << "'...");
        _io_work_->writer.reset(new datatools::data_writer);
        _io_work_->writer->init(output_file_name,
    ...
    if (_output_data_format_ == io_utils::DATA_FORMAT_BANK) {
      ...
      if (has_out_module()) {
        if (_io_work_->out_module->is_initialized()) {
          DT_LOG_NOTICE(_logprio(),
                        "Closing output module ('bank' data format)...");
          _io_work_->out_module->reset();
        }
        DT_LOG_NOTICE(_logprio(),
                      "Destroying output module ('bank' data format)...");
        _io_work_->out_module.reset();
      ...
      if (has_brio_writer()) {
        if (_io_work_->brio_writer->is_opened()) {
          if (_use_run_header_footer_) {
            _io_work_->brio_writer->select_store(
                _brio_general_info_store_label_);
...
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(mctools::g4::run_action, ocd_) {
  ...
  ocd_.set_class_documentation(
      "This is Geant4 simulation engine embedded run action. \n");
...

  {
    ...
    cpd.set_name_pattern("number_of_events_modulo")
        .set_terse_description(
            "The period to print the number of processed event")
...

  {
    ...
    cpd.set_name_pattern("file.save")
        .set_terse_description(
            "Flag to inhibit the saving of simulated data in some output file")
...

  {
    ...
    cpd.set_name_pattern("file.no_preserve")
        .set_terse_description(
            "Flag to allow the overwriting of some already existing output "
            "file")
        .set_traits(datatools::TYPE_BOOLEAN)
        .set_mandatory(false)
...

  {
    ...
    cpd.set_name_pattern("file.name")
        .set_terse_description(
            "The name of the file where to save the simulated data")
        .set_traits(datatools::TYPE_STRING)
        .set_path(true)
        ...
        .set_long_description(
            ...
            "  * ``.txt`` or ``.txt.gz`` or ``.txt.bz2`` : Boost ASCII "
            "archive\n"
            "  * ``.data`` or ``.data.gz`` or ``.data.bz2``: Boost binary "
            "archive\n"
...

  {
    ...
    cpd.set_name_pattern("file.directory")
        .set_terse_description(
            "The directory where the output data file is stored")
        .set_traits(datatools::TYPE_STRING)
        .set_path(true)
        .set_complex_triggering_conditions(true)
        .set_long_description(
...

  {
    ...
    cpd.set_name_pattern("file.using_run_header_footer")
        .set_terse_description(
            "Flag to activate the saving of special informations in run header "
...
DOCD_CLASS_SYSTEM_REGISTRATION(mctools::g4::run_action,
                               "mctools::g4::run_action")
