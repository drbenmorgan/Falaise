...
bool stepping_action::is_dumped() const { return _dumped_; }
...
std::string stepping_action::g4_track_status_to_label(
    G4TrackStatus track_status_) {
...
std::string stepping_action::g4_step_status_to_label(
    G4StepStatus step_status_) {
  switch (step_status_) {
    case fWorldBoundary:
...
std::string stepping_action::g4_stepping_control_to_label(
    G4SteppingControl stepping_control_) {
...
void stepping_action::_stepping_action_base(const G4Step* g4_step_) {
  ...
  if (process_step) {
    ...
    // const G4VTouchable * pre_step_touchable  =
    // pre_step_point->GetTouchable(); const
    // G4ReferenceCountedHandle<G4VTouchable> post_step_touchable_handle
    ...
    // const G4VTouchable * post_step_touchable =
    // post_step_point->GetTouchable(); G4ThreeVector post_position_t =
    // post_step_touchable->GetTranslation(); G4String process_name         =
    ...
    // G4double non_ionizing_energy_deposit =
    // g4_step_->GetNonIonizingEnergyDeposit(); G4SteppingControl
    // stepping_control   = g4_step_->GetControlFlag(); G4bool
    // first_step_in_volume   = g4_step_->IsFirstStepInVolume(); G4bool
    // last_step_in_volume    = g4_step_->IsLastStepInVolume(); G4ThreeVector
    // delta_position  = g4_step_->GetDeltaPosition(); G4double delta_time
    // = g4_step_->GetDeltaTime(); G4ThreeVector delta_momentum  =
    // g4_step_->GetDeltaMomentum(); // obsolete G4double delta_energy         =
    // g4_step_->GetDeltaEnergy();   // obsolete
    ...
    if (_dumped_) {
      ...
      std::cerr << "Dyn. part. charge   : "
                << dynamic_particle_charge / CLHEP::eplus << " [e]"
                << std::endl;
...
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(mctools::g4::stepping_action, ocd_) {
  ...
  ocd_.set_class_documentation(
      "This is Geant4 simulation engine embedded stepping action.\n");
...
DOCD_CLASS_SYSTEM_REGISTRATION(mctools::g4::stepping_action,
                               "mctools::g4::stepping_action")

/*
// Code sample that may be useful but care must be taken with the
// navigation through the depth of the G4TouchableHistory:

G4TouchableHandle pre_step_touchable_handle = g4_step_->GetPreStepPoint
()->GetTouchableHandle (); G4VPhysicalVolume * mother_volume = 0; int depth =
pre_step_touchable_handle->GetHistoryDepth(); clog << datatools::io::devel <<
"stepping_action::_stepping_action_base: "
<< "depth==" << depth << " volume=" << volume_name << endl;
if (depth < 1)
{
clog << datatools::io::devel << "stepping_action::_stepping_action_base: "
<< "depth==0 for " << pre_step_touchable_handle->GetVolume (0)->GetName () <<
endl;
